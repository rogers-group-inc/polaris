...
- **Telemetry** (default 60 s) — CPU, memory, and the full hardware-sensor table (temperature, fan, voltage, transceiver bias current and optical power, PSU). Vendor-specific SNMP profiles ship for Cisco, Juniper, Mikrotik, Fortinet, HP/Aruba, and Dell — they name the vendor's symbols, and the numbers come from the vendor's MIB you upload (see MIB Database); without it the profile falls back to HOST-RESOURCES-MIB and ENTITY-SENSOR-MIB.
