...
- **Conflict resolution** — discovery values that differ from an existing manual record become `Conflict` records for admin review. Asset conflicts cover hostname matches, NetBIOS-truncated matches, MAC collisions, duplicate Entra/AD registrations, two devices claiming one address, **one managed FortiSwitch/FortiAP on the rosters of two FortiGates** (whichever ran discovery last owns the record, so the card names both gates and when each last reported the device — Polaris changes nothing on the gates and closes the card itself once one of them stops), and **two asset records carrying one serial number** (one device recorded twice, resolved by merging); reservation conflicts cover sourceType/owner/notes drift; subnet conflicts report a **replaced FortiGate chassis** — a network whose serving gate answered with a different serial, with the old gate's reservations archived and offered for review so they can be carried onto the new box.
...
- **Saved filters & view tabs** — any combination of column filters + sort on the assets table can be saved as a named preset, kept **private** or published **public** for everyone who can view assets. The page also carries per-user **tabs**, each holding its own filters, its own starred rows and its own **column order**, so several views stay open side by side — arrange the columns a firewall view is about without disturbing an addressing view. Tabs are renameable, a preset can be opened straight into a new tab, and both presets and tabs are stored server-side so they follow the operator across browsers (column *widths* and hidden columns stay per browser, since those are a property of the screen).
...
- **Telemetry** (default 60 s) — CPU, memory, and the full hardware-sensor table (temperature, fan, voltage, transceiver bias current and optical power, PSU). A **manufacturer profile** names which symbol is the CPU and which pair is the memory, and exists to override what the generic MIBs already do — so one ships only where there is something to override, and only for a manufacturer whose MIB ships too (Cisco today, on fresh installs). For every other vendor you upload the MIB and create the profile once; `docs/INSTALL.md` lists each vendor's symbols. Without a profile the stream falls back to HOST-RESOURCES-MIB and ENTITY-SENSOR-MIB, which is also what happens if you delete one.
...
A managed FortiSwitch or FortiAP additionally carries a **controller link** state, swept from its parent FortiGate every 60 s: does the gate still hold the FortiLink session to that switch, the CAPWAP tunnel to that AP. It is deliberately separate from the monitor status pill, and the two are allowed to disagree — that is the point. A switch whose FortiLink session has dropped still answers every ping and every SNMP get, so it reads **Up** on the pill while the gate has quietly stopped managing it: no switch-controller config reaches it, no VLAN change lands. The row appears on the asset's General tab with how long it has been in that state, and `Controller link (FortiLink / CAPWAP)` is an automation trigger like any other, so you can be emailed or paged on it.

Two things it will not do. A controller Polaris cannot reach writes **nothing** rather than reporting everything behind it as down — the row then says when the value was last confirmed instead of presenting a stale reading as current. And a device the controller answers about but does not list reads **Unknown**, not Down, because that is also what a brief config-push window looks like. Cost scales with how many FortiGates you have rather than how many devices they manage; `POLARIS_FORTILINK_SWEEP_SEC` slows it down where FortiManager proxy mode makes that worth doing.

Operators can pin specific interfaces, mountpoints, or IPsec tunnels for **sub-minute polling** without re-walking the full table, and pinned entities keep full tiered history. Unpinned mountpoints and IPsec tunnels are still sampled on the slow tier and retained 24 h; an **unpinned interface is not sampled at all** — its current state (status, speed, counters, IP/MAC, VLAN) is kept in a live inventory table instead, so it has no chart until you pin it. Pinning is also what makes a sub-asset **alertable** — an automation produces readings only for pinned interfaces, mountpoints and tunnels (business rule 57), on every one of them for the same reason rather than as a side effect of what gets sampled: un-pinning is how alerting on one stops, and a device with nothing pinned raises no interface, storage or tunnel alerts at all. Each FMG/FortiGate integration carries per-stream **REST ↔ SNMP toggles** (response time, telemetry, interfaces, LLDP) so branch-class FortiGates whose REST sensor endpoints 404 on FortiOS 7.4.x can be moved to SNMP one stream at a time. Per-asset overrides take precedence when set.
...
### MIB Database
Admin-uploaded SNMP MIB modules drive vendor-specific telemetry. Uploads are validated by a minimal SMI parser (real ASN.1 modules only — binaries and arbitrary text are rejected) and scoped three ways: **Manufacturer-wide** (the common case), **Device-specific** (overrides one model), or **Generic** (shared across vendors). Resolution priority at probe time is *device → vendor → generic → shipped standard → SMI seed*. Polaris **ships no vendor OIDs in code** — a vendor's telemetry resolves by symbol name out of a MIB — and ships MIBs two ways. The IETF/IEEE standard modules (IF-MIB, HOST-RESOURCES-MIB, ENTITY-MIB, LLDP-MIB, BRIDGE/Q-BRIDGE, IP-MIB…) are **baked in**: present in every install, deletable by nobody, changed only by a product update. A manufacturer's own MIB is **seeded** instead — Cisco's three modules are loaded into the MIB Database on the first start of a fresh install, where they sit alongside your uploads and **you can delete them**; an upgraded install receives none, because an existing MIB Database is yours. Every other vendor's telemetry resolves once you upload that vendor's MIB, including its core module (FORTINET-FORTIGATE-MIB imports `fortinet` from FORTINET-CORE-MIB; Fortinet ships both in one download, and a FortiGate serves them from its own System → SNMP page). The upload response says how many symbols resolved and names any module still missing, and each row on the Manufacturer Profiles card shows which MIB its symbol resolves from — or exactly what to upload when it does not. Each row has a **Browse** button (open to admin and assets-admin) that opens a two-pane modal listing every object the MIB defines — Tables and Scalars / Other — with type, access, and description for each, plus a **Walk on asset…** pivot that picks an SNMP credential and runs the walk against any asset, returning symbolic names with decoded INTEGER enums (`up(1)` instead of `1`) and 2D table rendering when the walk lands on a SMI table.

### Firmware repository (switches and access points)
Server Settings → **Repository** files firmware images under a manufacturer → device type → model tree built from the inventory. A model keeps two images (a **primary** and a **backup**; uploading rotates them, and either can be made primary), and a device-admin login is bound at model, device-type or manufacturer level with the most specific winning. Every image is matched to a device by the **platform token in its own header** — the first six characters of the device's serial — never by the model string, and only ever forward: a device's Firmware card offers an upgrade when the repository holds a strictly newer image for its platform, and the operator approves that exact image (version, build, platform, file, hash) before anything is pushed. The flash runs over HTTPS to the device's own web UI (Fortinet FortiSwitch and FortiAP today), takes a maintenance hold on the device so the alerts behind it stay quiet, reports erase / write / verify progress live, and records the version it reads back after the reboot; the asset's own record is updated by discovery, never by the flash. An informational baseline automation flags every switch or access point whose running firmware differs from its platform's primary. Starting an upgrade is its own permission rung (Full Read-Write on *Firmware Repository*), separate from managing the repository. **Bench-test on non-production hardware before fleet use** — a flash that fails partway can leave a device unbootable.
...
- **Passkeys (WebAuthn)** — local accounts can register a passkey and sign in with a PIN, fingerprint or security key instead of a password. An admin chooses what a passkey is for: sign-in, a second factor, both (the default), or off. Requires HTTPS and a hostname — the UI says so where it cannot be used.
- **Password complexity policy** — minimum length and which character classes are required, set under Users → Authentication → Settings. Defaults to the rules Polaris has always enforced (8 characters, upper + lower + number + symbol), and can optionally require existing users to replace a non-conforming password at their next sign-in.
- **Local login access** — optionally restrict the local login form (`/login.html`) and the password endpoints to chosen source networks (RFC1918 + loopback, all, or a custom CIDR allow-list), under Users → Authentication → Settings → Trusted Networks for Login. Off by default. SSO sign-in is never restricted; the local *and* LDAP password path is. Disallowed page requests are dropped and credential posts get the same generic 401 a wrong password does. Enabling it is refused if it would exclude your own address, since `/login.html` (`/login.html?local=1` while "Skip login page" is on) is how you get back in when the identity provider is down.
- **Roles** — Admin, Network Admin, Assets Admin, User, Read-Only, plus custom roles built from a 33-function permission matrix. Network and asset surfaces are role-scoped; **users own the records they create** — networks, reservations, address-book entries, saved Discoveries and stored credentials are all editable by their author (Read-Write) or by anyone holding Full Read-Write on that function. **Each function offers only the levels that mean something for it**, so most stop at Read-Write and a few offer just one level: an asset probe dials the device and changes nothing in Polaris, so Read is its whole grant, while nothing under Server Settings → Data is merely viewable, so that one has no Read at all. Two acts are deliberately held one level higher than the surface they sit on — **deploying the Polaris Agent** (it runs an installer on the host and leaves a service behind, so it needs Full Read-Write on Assets, on the per-device buttons and on the integration's fleet-wide auto-deploy toggle alike) and the dependency-down **simulation**, which can briefly mask a real outage.
...
Optional, RHEL only, and planned rather than bolted on afterwards: `deploy/ha/setup-rhel-ha.sh` builds the Patroni + etcd cluster described under [High availability](#high-availability-optional) above, and Server Settings → **High Availability** drives it from the UI. Read [docs/HA.md](docs/HA.md) first — the witness placement decides which outage fails over automatically, and both nodes must serve the identical nginx certificate because enrolled agents pin that exact leaf.
...
- **Lifecycle** — disabled devices import as `decommissioned` or are skipped, and both directory integrations can optionally **decommission the assets they manage** when a device is deleted from the directory or disabled in it (off by default) — guarded so that a truncated or empty read, a filter change, or an implausibly large drop refuses the pass and logs why rather than acting on it. See business rule 70.

### Active Directory (on-premise)
A domain controller via LDAP / LDAPS simple bind, read-only domain user. Produces **assets only** — computer objects under a configured base DN, mapping hostname, DNS name, OS / OS version, OU path, `whenCreated`, `lastLogonTimestamp`, and description. Disabled accounts can be imported as `decommissioned` or skipped. Both directory integrations can optionally **decommission the assets they manage** when a device is deleted from the directory or disabled in it (off by default) — guarded so that a truncated or empty read, a filter change, or an implausibly large drop refuses the pass and logs why rather than acting on it. See business rule 70. Wildcard OU include/exclude filters. Discovery reads **computer objects only**; the bind account needs read access to user, group and contact objects as well **only** if you enable one of the two address-book directory features on the integration's **Directory** tab. Both are opt-in and default off: **search** is a live typeahead lookup that stores nothing, while **sync** writes each matching person into the Polaris address book — name, email, job title, department and phone — where it also lands in every backup. See business rule 35.
