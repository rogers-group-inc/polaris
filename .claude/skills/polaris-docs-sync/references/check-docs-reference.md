...
| `paths-exist` | a concrete `src/ public/ prisma/ scripts/ deploy/ docs/ tests/ .claude/skills/` path named in a doc does not exist on disk | fix the path or delete the dead pointer; templated paths with `<>` or `*` are ignored |
...
| `rule-anchors` | in `polaris-business-rules`: an invariant line (`N. **`) for a rule above 11 does not end with a `→ [narrative-X.md#rule-N](narrative-X.md#rule-N)` pointer, the pointer's text and target disagree, the named narrative file is missing or has no `## Rule N` heading, a rule has invariant lines in two files, a `SKILL.md` index row names an invariant or narrative file that does not hold that rule, or a narrative `## Rule N` heading has no invariant line | fix the pointer or the heading; a new rule adds all three layers at once (invariant line, `narrative-N.md` with its `## Rule N —` heading, index row). Rows whose file cells are `—` (retired / gap / in-flight) are skipped |
...
- **`tests/` joined the path roots on 2026-09-21**, after a renamed test file left a dead `[tests/unit/…](tests/unit/…)` link in `canon-tables-lists.md` that a full green `check:docs` run sailed past — the canon files cite their guarding test by path constantly, so that was the one class of pointer nothing verified. If a doc has to name a test that does not exist yet, write it templated (`tests/unit/<name>.test.ts`) the way every other illustrative path is written.
