...
| business rule | `polaris-business-rules` — `npm run rules:next` for the number; a ~150-word invariant appended to `invariants-30-43.md`, a new `narrative-N.md` (one file per rule), an index row in its SKILL.md; cite the number from code and the pinning test. `check:docs` (`rule-anchors`) fails when the three layers disagree |
| **anything an operator sees or configures** — a page, tab, card, modal, widget, wizard step or control under `public/`; a permission key or a route gate; an integration config field or toggle; an automation trigger / action / recipient kind; a polling method or stream; a Server Settings surface; a business rule's operator-visible behaviour | the matching page under **`docs/wiki/`**, which is the source for the GitHub wiki the account menu's **Help** entry opens (`WIKI_URL` in `public/js/app.js`). `docs/wiki/README.md` carries the file→page mapping and the publish procedure. The page map is: screens → `Dashboard` / `IPAM` / `Assets` / `Device-Map` / `Application-Map` / `Events` / `Automations` / `Integrations` / `Users-Roles-and-Permissions` / `Server-Settings` / `Mobile-and-Dash`; automations → `Automation-Triggers` / `Automation-Actions` / `Automation-Escalation` / `Automation-Scripts` / `Delivery-Channels` / `Address-Book`; monitoring → `Monitoring` / `Polling-Methods` / `Monitor-States` / `Dependency-Suppression` / `Maintenance-Windows`; discovery → `Discovery` / `Integration-*` / `Polaris-Agent` / `Network-Discovery`; reference → `Conflict-Resolution` / `Business-Rules` / `API` / `Backup-and-Restore` / `Updates` / `High-Availability` / `Troubleshooting`. A **new rule** gets a `### Rule N` heading in `Business-Rules.md` (the heading text is exactly `Rule N` — it is the `#rule-N` anchor every other page cites). A **new page** is also added to `_Sidebar.md`, or it is unreachable. Links between pages are bare page names, never paths or `.md` suffixes |
| `public/api.html` (the external API contract) | **two places, both required.** (1) `docs/wiki/API.md`, which mirrors the same endpoint list and conventions for readers with no install in front of them — it is a mirror, so an endpoint added to the page and not to it is a wiki that documents a narrower API than Polaris has. (2) regenerate the `polaris-api-conventions` plugin. The clone lives beside this checkout at `../polaris-api-conventions` (do not re-clone; origin https://github.com/rogers-group-inc/polaris-api-conventions). From the folder holding both clones: `node polaris-api-conventions/scripts/import-api-html.mjs polaris/public/api.html`, then in the plugin repo bump `plugin.json` version, commit, push. `check:docs` compares its `sourceApiHtmlSha256` against the current page and warns (never fails) when they have drifted — silence means current, or that the clone was not found |
...
| the automation SCRIPT surface (a new interpreter, a new `{token}`, a changed limit or `POLARIS_*` context var — `services/automationScriptRunner.ts`, `automationScriptService.ts`, `utils/notificationTemplate.ts`, `agent/internal/scriptexec/`) | mirror it in the `polaris-automation` plugin, cloned beside this checkout at `../polaris-automation` (origin https://github.com/rogers-group-inc/polaris-automation). Its `polaris-automation-scripts` skill generates pasteable scripts from facts read out of those exact files, so a change there silently invalidates them: amend the reference, bump `plugin.json`, commit and push in that repo. The TRIGGER vocabulary is not its surface — it is about what a script may do once an automation fires |
| `design/css/` re-synced from the external kit (a `docs(design): sync the kit` commit), or a preview card / the conventions header edited | run `/design-sync` — the Claude Design project is a manual push that nothing re-runs, so it goes stale silently. `.design-sync/NOTES.md` owns the trigger table, the upload procedure and the validation pass, and `/design-sync` reads it before acting. A `public/css/` change on its own is NOT a trigger: `design/` tracks the external kit and is never edited to chase `public/` |
