...
#   POLARIS_MONITOR_SDWAN_WORKERS     (default 8)  — FortiOS SD-WAN health checks + rule selection (`polaris-monitor-sdwan`, light loop, per-integration `sdwanIntervalSeconds`)
...
#   POLARIS_FORTILINK_SWEEP_SEC       (default 60, floored at 30) — seconds between controller-link sweeps for FortiGate-managed FortiSwitches / FortiAPs (business rule 59): does each controller still hold a FortiLink session to its switches, a CAPWAP tunnel to its APs. Not a worker pool — the sweep is one pass over controllers, and its cost tracks CONTROLLER count rather than fleet size (≤2 upstream calls per controller per tick whether it manages 3 devices or 300). The install to think about is FMG PROXY mode, which serializes at concurrency 1: ~100 proxied calls per minute on a 50-gate fleet at ~650ms each. Direct mode (useProxy=false) and standalone FortiGate integrations talk to each gate independently and have no such ceiling. Where the REST probe path already reads the same controller the two share a 30s inventory cache, but only when their independent 60s timers land in one window — budget ~1.5 calls/min per kind there, not 1. The 30s floor is that cache's own TTL; anything faster would simply miss it. Raising it slows link-state detection proportionally, since a poll-counted automation hold counts SWEEPS ("down for 3 polls" at 300s is 15 minutes)
