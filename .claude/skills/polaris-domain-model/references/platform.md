...
UserTableTabs                   -- Per-user list-page TABS — this operator's set of open views on one table
  ...
  tabs          Json            -- { version: 1, tabs: [{ id, name, state, savedFilterId?, savedFilterName?,
                                ...
                                --   favoriteIds / columnOrder are this tab's starred rows and its movable-column
                                --   order; both distinguish null (absent — the client may seed once from this
                                --   browser's localStorage) from [] (deliberately empty).
  updatedAt     DateTime
...
MibFile                         -- Admin-uploaded SNMP MIB modules used to resolve vendor-specific OIDs during monitoring
  ...
  -- Uploads are not the only MIBs the resolver knows. Since 2026-09 `oidRegistry` lays the bundled IETF/IEEE standard modules (`src/services/stdMibs/`) down as a STANDARD layer beneath every upload — resolution order is device → vendor → generic → standard → seed — so `ifHCInOctets` resolves by name on an install that has uploaded nothing, and an uploaded generic module of the same name (a newer IF-MIB) overrides the shipped one. The standard layer has no rows in this table: it is read from disk once per process, and its symbols report `fromScope: "standard"` with the module name. The seed beneath it is SMI scaffolding only (`iso`, `enterprises`, `mib-2`, the IEEE 802.1 chain) — vendor OIDs come from rows here (see polaris-change-impact → cross-cutting/vendor-snmp-knowledge-boundary).
...
ManufacturerProfile             -- Operator-editable per-manufacturer telemetry profile. Replaces the hardcoded VENDOR_TELEMETRY_PROFILES constant at runtime; the constant stays as a fresh-install fallback until the resolver swap lands. Seeded idempotently from the constant on first boot via `seedManufacturerProfiles` startup job.
  ...
  matchPattern  String?            -- "Also applies when": operator-owned case-insensitive regex tested against `${manufacturer} ${os} ${pinnedMibModuleName}`, consulted ONLY when no profile is keyed by the asset's canonical manufacturer. Replaces the hardcoded vendor constant's `match` alternations for the three cases the unique key cannot express: an alias spelling the seed did not key ("Aruba", "HPE" → the HP profile), OS-only identity (SNMP reports "Cisco IOS" and no manufacturer), and a MIB-pin redirect. Validated like `modelPattern` at write time (PUT /manufacturer-profiles/:id). Seeded profiles are backfilled with the constant's alternations by migration `20260915020000_manufacturer_profile_resolver_swap`; blank is the normal state for an operator-made profile, and blank means no second chance — not "matches everything".
...
ManufacturerProfileMetric       -- One row per System-tab metric the profile owns: model / cpu / memory / temperature / interfaces / storage / lldp / wirelessStations
  id              UUID PK
  profileId       UUID FK → ManufacturerProfile (cascade)
  metricKey       String           -- "model" | "cpu" | "memory" | "temperature" | "interfaces" | "storage" | "lldp" | "wirelessStations". `METRIC_KEYS` in `manufacturerProfileService` is the canonical order and `shapeProfile` sorts every read by it, so a row a migration appends still renders in sequence; the client mirror is `METRIC_KEY_LABELS` in `public/js/server-settings.js`. **`model`** (added 2026-09) is the IDENTITY query — which vendor scalar carries the hardware model and how to parse it — the last profile fact that could only live in code.
  ...
  defaultAggregate String          -- "none" | "avg" | "sum" (default "none"). How a WALKED subtree collapses to ONE reading. Kept apart from `defaultType` because the axis is orthogonal — Cisco memory is `double_scalar` AND walked+summed, Juniper memory `scalar` AND averaged, FortiSwitch memory `double_scalar` and not walked. Replaces the hardcoded profile's `walkSubtree`, which the editable rows could not express. Surfaced on the cpu / memory rows only.
  defaultLabel     String?         -- Label for the sample row a scalar-form metric synthesizes: the `StorageSample.mountPath` ("flash" on a FortiSwitch — the key pins and thresholds attach to) or the hardware-sensor `sensorName` ("System" for a FortiAP's single reading). Null → the collector's default. Surfaced on the storage / temperature rows only.
  defaultParsePattern  String?     -- `model` metric ONLY: a regex with one capture group, applied to the vendor scalar's raw string.
  defaultParseTemplate String?     -- `model` metric ONLY: a `$1` template turning that capture into `Asset.model` — `"S548DF-v7.2.5-…"` → `"FortiSwitch S548DF"`. Pattern + template together replace the JS parse FUNCTION the hardcoded FortiSwitch profile carried; `src/utils/modelParse.ts` owns the evaluation and `validateModelParse` is the write-time gate (including the "don't re-prefix a token that already carries the prefix" rule read off the template). The service rejects parse fields on any metric other than `model`.
  defaultTransform String?         -- For `scalar`, a unary `TransformKind` restricted per metric key to `METRIC_ROW_TRANSFORMS` (today only `tenths_to_units` on `temperature`, applied by the hardware-sensor collector; there is no Celsius↔Fahrenheit transform — removed 2026-09-23); for `double_scalar`, a binary `CombinerKind` (`a_over_b_as_percent` / `a_over_a_plus_b_as_percent` / `b_minus_a_over_b_as_percent` / `a_minus_b` / `a_plus_b` / `a_over_b_ratio`). Null on `table`, optional on the other two. Validated at write time against the matching registry in `src/utils/symbolTransforms.ts` based on `defaultType` — a unary transform on a `double_scalar` row (or a combiner on a `scalar` row) is a 400 at the service layer.
...
ManufacturerProfileMetricOverride -- A SCOPED row inside one metric row: a device-type default, a per-model exception, or a per-model exception under one device type
  id             UUID PK
  metricRowId    UUID FK → ManufacturerProfileMetric (cascade)
  assetType      String?           -- Scope half one: an `AssetTypeDef.name`, registry-validated at write time (`isKnownAssetType`, so a custom type an operator added can carry a default too), or null for "any device type".
  modelPattern   String?           -- Scope half two: regex matched against Asset.model (+ the pinned MIB's module name), or null on a device-type default. **At least one half is always set** — DB CHECK `manufacturer_profile_metric_overrides_scope_check` plus `assertOverrideScope` in the service, which validates the EFFECTIVE pair (input merged over the stored row) so clearing the pattern on a type-scoped row PROMOTES it to that type's default and clearing it on an any-type row is refused. One type default per `(metricRowId, assetType)` is enforced by the PARTIAL unique index `mfg_profile_metric_overrides_type_default_key` (WHERE modelPattern IS NULL) — SQL only, since Prisma cannot express a partial index and a full `@@unique` would forbid two model exceptions under one type; its P2002 becomes a 409 naming the type.
  symbol         String            -- For `type="double_scalar"` this is the "A" OID; for `scalar`/`table` it's the single OID.
  aggregate      String            -- Mirror of `ManufacturerProfileMetric.defaultAggregate`.
  label          String?           -- Mirror of `ManufacturerProfileMetric.defaultLabel`.
  parsePattern   String?           -- Mirror of `ManufacturerProfileMetric.defaultParsePattern` (`model` metric only).
  parseTemplate  String?           -- Mirror of `ManufacturerProfileMetric.defaultParseTemplate` (`model` metric only).
  ...
  transform      String?           -- TransformKind on scalar (same METRIC_ROW_TRANSFORMS restriction as the row); CombinerKind on double_scalar; null on table.
  ...
  -- Resolution is MOST-SPECIFIC-FIRST: type+model → model → type default → the metric row's own defaults, with `order` breaking ties inside a tier (lower wins). A stated model outranks an inferred type on purpose, so a mis-typed asset whose model names the family still routes. The pre-2026-09 shape (modelPattern only) is unchanged and still legal; the device-type tier replaces keying a device FAMILY off a model regex ("FortiSwitch"), which matched an empty model only through the Fortinet-specific `fortinetClassHint`. FOUR metric keys are actually read by the runtime — `cpu` / `memory` / `temperature` / `storage` (the last since 2026-08; `interfaces` / `lldp` / `wirelessStations` are table walks with no symbol to swap and stay descriptive on the profile page). Seeded entries cover the Fortinet sub-families: `FortiSwitch → fsSysCpuUsage / fsSysMemUsage / fsSysDiskUsage`, `FortiAP → fapCpuUsage / fapMemoryUsage / fapTemperature`. The FortiSwitch memory override is stamped as `type="double_scalar", symbol="fsSysMemUsage", symbolB="fsSysMemCapacity", transform="a_over_b_as_percent"` so the editable profile matches the hardcoded baseline's bytes-form semantics. Existing installs receive the promotion via the `backfillManufacturerProfileMemoryComposition` startup job (kept its legacy name for marker-key back-compat).
