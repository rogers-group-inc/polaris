...
| how a polling method is resolved (asset → class → integration per-class → integration flat → manual → floor), compatibility vs capability, agentless host streams, credential resolution, where a vendor OID or MIB symbol is allowed to live (the no-enterprise-OID boundary) | [references/monitoring-architecture-polling.md](references/monitoring-architecture-polling.md) |
