...
### Rule 80 — the invariant as stated in full until 2026-09-22
...
### Rule 80a — the invariant as stated in full until 2026-09-22
> Moved here verbatim from the invariants file on 2026-09-22, when the invariant layer was cut back to the contract alone; the short invariant now points here for the reasoning and the dated history. Nothing below was rewritten.

**A silence is granted for when the event HAPPENED, not for when something got round to reading it** — the event tail applies a SECOND gate beside `assetCanTrigger`: was this asset inside a maintenance window at `ev.timestamp`, read from `AssetMaintenanceWindow` history (`services/notificationEngine.ts` → `loadMaintenanceSpansForEvents()` + `suppressedAtEventTime()`). `assetCanTrigger` reads the LIVE Asset row, which is right for a threshold rule — a reading is current by definition — and wrong for the tail, which works off a backlog on a job that runs **once a minute**: an event is judged up to 60 s after it happened, so **a maintenance window shorter than that tick could not suppress anything at all**. Rule 80 made it total rather than intermittent: an agent upgrade's window is about TWO SECONDS wide (`maintenance.entered` 10:30:47, `agent.disconnected` 10:30:49, `maintenance.exited` 10:30:49), so the hold worked perfectly, the window was long shut when the tail looked, and the operator was paged for exactly the work they asked for. It is why the rule's own trials passed — a SLOW operation is still inside its window when the tick lands. The defect is general, not an agent carve-out: a scheduled window that ends quickly, or an operator releasing maintenance shortly after an event, leaked identically. Four things hold it together. **One query per tick**, keyed by the assets in the BATCH and bounded by its span, never one per event. **Both boundaries inclusive** — at two seconds wide, an event stamped the same millisecond the window closed is the ordinary case. **No grace after `endedAt`**, although `Event.timestamp` is `@default(now())` (stamped by the DATABASE at insert) while `detach()` does not await its write, so a loaded host could land the insert just after the close: a grace would also swallow `agent.upgrade_failed`, which `failUpgrade` writes IMMEDIATELY after dropping the hold so a dead agent still alerts, and losing that beats the rare race, which merely degrades to one late alert. **Resets are not gated** — reset matchers are collected before the trigger gates, so `agent.connected` still clears the alert; suppressing a reset would strand a live one. Does NOT cover `dependencySuppressed` (a live boolean with no history to consult) nor the `monitored` half, which is correctly read live — stopping monitoring is not an interval and must not deliver the last minute of a device's events.
