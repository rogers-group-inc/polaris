...
it reports the proxy clean. A scan harness that fronted a dev instance with the shipped
directives existed for exactly this until 2026-09-23, when the HawkScan setup was removed from
the repo; anything that re-verifies the edge half must put the shipped nginx config in the
response path the same way. The one setting such a front must not omit is `TRUST_PROXY=1`, without which Express
