...
description: "The 44 numbered Polaris business rules — each invariant and the incident that forced it. Load BEFORE changing any behavior around subnets/CIDR overlap, reservations, DHCP leases or bindings, discovery writes to assets, lastSeen, monitorStatus (up/down/warning/recovering/passive), dependency suppression, maintenance windows, automations/alerts/notifications/escalation/acknowledge/reset, packet loss, secrets at rest, backups, SSH host keys, login restriction, RBAC grant levels, tags/regions, placeholder MACs, Windows OS names, logos; whenever code, a commit or a doc cites 'business rule N' / 'rule N'; and when asked to add or retire a rule."
...
| touch discovery writes (assets, descriptions, locations, ARP, MACs) | 13, 14, 15, 17, 22, 26, 28, 35, 40, 41, 44 |
...
- [references/invariants-30-44.md](references/invariants-30-44.md) — the one-paragraph invariant for rules 30–44
...
- [references/narrative-36-44.md](references/narrative-36-44.md) — full narrative, rules 36–44
...
| 30 | Confirmation is the configured cadence's job; ICMP only fills in the loss ratio | invariants-30-44 | narrative-25-35 |
| 31 | The login page is the way back in, so restricting it is opt-in and must refuse to lock you out | invariants-30-44 | narrative-25-35 |
| 32 | A reset condition starts as the trigger inverted and resolves where the alert lives | invariants-30-44 | narrative-25-35 |
| 33 | A device that answers is not a device that works; the check belongs to the VENDOR | invariants-30-44 | narrative-25-35 |
| 34 | An active scan finds things; a separate grant adds them | invariants-30-44 | narrative-25-35 |
| 35 | The GAL is a mirror, not a source of truth; Polaris only deletes what it wrote | invariants-30-44 | narrative-25-35 |
| 36 | The automation decides what "down" means; a device no automation covers is never judged | invariants-30-44 | narrative-36-44 |
| 37 | An automation only fires about a device Polaris is actually polling | invariants-30-44 | narrative-36-44 |
| 38 | A device behind a dark parent is not accused, and is not released on one packet | invariants-30-44 | narrative-36-44 |
| 39 | How a person wants to be reached is theirs, not the automation's | invariants-30-44 | narrative-36-44 |
| 40 | Two assets on one address is a conflict; one asset on a stale address is not | invariants-30-44 | narrative-36-44 |
| 41 | A subnet dies with its FortiGate; the chassis, not the name, says which gate | invariants-30-44 | narrative-36-44 |
| 42 | Some address space is not one network; the way to say so is to exclude it | invariants-30-44 | narrative-36-44 |
| 43 | A grant is only as narrow as the act it names | invariants-30-44 | narrative-36-44 |
| 44 | An address places a device only through the gate that owns it, and only a device nothing else can place | invariants-30-44 | narrative-36-44 |
