...
(79 is the next free number. Three open worktrees hold the gap: 75 `worktree-alert-grouping` (grouped alerts); 76 `worktree-ssh-firewall-domain-profile` (OpenSSH built-in rule) AND `worktree-vip-discover-status` — the second of those to merge renumbers to 77; 78 `worktree-dependency-down-alerts` (dependency-down alerts).)
...
- [references/invariants-30-43.md](references/invariants-30-43.md) — the one-paragraph invariant for rules 30–78 (the filename keeps its original range: the reference is cited from code and the other skills)
...
- [references/narrative-60-64.md](references/narrative-60-64.md) — full narrative, rules 60–78 (the filename keeps its original range: it is a stable citation key. Split out 2026-09-15 when the 44–48 file reached the 1500-line ceiling)
...
| 78 | An automation may choose to speak for a silenced device, and then it must name who silenced it | invariants-30-43 | narrative-60-64 |
