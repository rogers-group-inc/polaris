...
(82 is the next free number. **75 is NOT free** — it is claimed by an in-flight worktree that
has not merged yet. 77 and 78 were both in that position and landed on 2026-09-21; 80 and 81
are this file's own newest rules. `main` alone will tell you none of that: it is exactly the collision the
...
| touch discovery writes (assets, descriptions, locations, ARP, MACs) | 13, 14, 15, 17, 22, 26, 28, 35, 40, 41, 45, 55, 79, 81 |
| touch whether discovery may REMOVE a field it can no longer read — a stale IP, blank-fill vs. strip | 81 (and 12 for why a cached read is not evidence) |
...
- [references/narrative-81.md](references/narrative-81.md) — full narrative, rule 81 (one file per rule, continuing the split above; the filename is a stable citation key)
