...
| `public/js/assets-maintenance.js` | `PolarisRecurrence` | one-line summary of a RECURRENCE shape (`{kind: oneshot\|recurring, freq, daysOfWeek, startTime, endTime, activeFrom, activeUntil}`) plus the weekday vocabulary — read by the Maintenance modal that owns it AND by the automations wizard's quiet-time editor (business rule 44), which stores the same shape. Two summarisers of one shape drift into describing the same window two ways on two pages |
