...
description: "Polaris frontend (vanilla JS, no build step, public/): which file is the reference implementation of each UI pattern (SVG charts, modals, stacked modals, slide-overs, wizards, sortable/filterable tables, column layout, row context menus, condition builder, widgets, mobile sheets), the page → permission-gate table, the shared modules never to re-derive, the three themes and the transit palette the theme band / theme strip sweep through, list-page and modal conventions, and the kit contract in design/POLARIS-UI-GUIDE.md. Load for any change under public/, any HTML/CSS/JS/UI/UX/theme/theme band/theme dial/theme strip/theme picker/dark mode/light mode/dashboard/widget/chart/mobile PWA/Dash wallboard request, anything about the sidebar or nav bar and the status panels, indicators and dots that live in it, anything about the Device Map or Site Map surfaces and the OpenStreetMap basemap they draw — the tile URL, the tile host, blocked or missing tiles, the tile usage policy — anything about a screenshot, a camera button, a capture or an image copied to the clipboard from a tab, a table or a chart, and what it did or didn't come out looking like — or 'make it look like the X page'."
---
...
| anything on the phone SPA (bottom sheet, a sheet other sheets stack on, pull-to-refresh, a list tab's filter + sort toolbar, keyboard fit, the theme strip) | [references/canon-mobile.md](references/canon-mobile.md) |
...
| html-to-image screenshots; the Device Map topology graph — its layout solver AND which theme family its Cytoscape palette takes; or the OpenStreetMap basemap — the single tile host every map surface must use, and why tiles come back "Access blocked" | [references/tech-stack-frontend.md](references/tech-stack-frontend.md) |
...
Vanilla JavaScript SPA served from `/public/`. **No build step** — plain ES modules, multi-page layout with client-side navigation in `app.js`, **three SELECTABLE themes in two families** (`morning` / `noon` daylight, `nightfall` dark — the retired `dark`/`light` ids are recognized nowhere; branch on `isLightTheme()`, never on an id) plus the `afternoon` **transit palette**, a real token block with a position on the strip and no `THEMES` entry that the sidebar band travels THROUGH on noon → nightfall and never persists, per-user panel lock on every modal and slide-over. Modals, slide-overs and menus are **frosted glass** — a translucent mix of the theme's own ground under a shared `backdrop-filter`, with the daylight pair mixing thinner than the dark one; the token set and the four things that silently break it are in polaris-ui-canon → canon-shared-kit.md § Floating-surface tokens. Never give a new floating surface a background of its own. A surface that merely rests ON the page takes an elevation token instead, by what it is: `--shadow-card` for a big opaque content box (every card, the dashboard widgets, the map and graph canvases, and `.chart-box` — the plot surface every SVG chart in the asset slide-over is drawn into), `--shadow-control` for small chrome, `--shadow-md` for a menu or dropdown. `--shadow-sm` is retired — it painted the cards flat on the daylight pair.
...
| Connections | `connections.html` | `connectivityChecks:read` (agent-run connectivity checks; sidebar entry directly under Application Map) |
...
| `public/js/dashboard-saved.js` | `PolarisSavedDashboards` | the "Dashboards ▾" menu on BOTH the Dashboard page and the Dash wallboard — save / publish / load a named canvas, and (wallboard only) pin a published one. Its counterpart seam is `window.PolarisDashboard` in `dashboard.js` — which is also how a widget HEADER control writes the saved layout (`setWidgetConfig(id, key, value)`, the ⇅ sort button's path), since `widgets/index.js` is handed a body element and knows nothing about the canvas |
...
| `public/js/mobile/app.js` | `PolarisTheme` | the phone's theme: `get()` answers the FAMILY (map-tab's basemap, topology-tab's palette), `set()` writes a real id and never persists a transit one, `advance()` is the strip's one-tap sweep, `seatStrips()` positions a strip rendered after boot. Its `MOBILE_THEMES` / `MOBILE_TRANSIT_THEMES` / `THEME_STRIP_POS` move in lockstep with the desktop's `THEMES` / `TRANSIT_THEMES` / `THEME_BAND_POS` — one clock, one geometry, two implementations, because neither document loads the other's JavaScript. `THEME_STRIP_POS` and `THEME_BAND_POS` are now the SAME four numbers and `tests/unit/themeBandParity.test.ts` fails when they stop being |
| `public/js/mobile/list-controls.js` | `PolarisListControls` | the phone's list filter field + "Sort & filter" sheet (Assets, Networks). Draws and reports only — a paged list sorts server-side, a whole list in the browser; the conventions are in polaris-ui-canon → canon-mobile.md § Mobile list toolbar |
...
| `public/js/password-self.js` | `PolarisPasswordSelf` | the password-complexity checklist (`rulesHTML`/`check`/`matchHTML`/`checkMatch` — users.js forwards to these) + the self-service Change Password modal. Since the bar became operator-configurable the checklist FETCHES which rules apply (`GET /auth/password-policy`, once per page) and repaints every rendered container when it lands, while shipping the five predicates so a list drawn before that request is still honest. Also loaded on `login.html` and `mobile.html`, where the forced-change step uses the same checklist |
| `public/js/totp-self.js` | `PolarisTotpSelf` | self-service TOTP enroll/confirm/disable modals |
| `public/js/webauthn.js` | `PolarisWebAuthn` | the browser half of a passkey ceremony — base64url ↔ ArrayBuffer and the two `navigator.credentials` calls, wrapped so they take and return the JSON shapes the server speaks. No fetch, no DOM, no api.js dependency, because `login.html` deliberately does not load the API client and every other page does. `supported()` is the gate every passkey affordance checks: it requires `isSecureContext`, since Polaris supports plain-HTTP installs where the API exists and every call rejects. `unavailableHere(rpId)` is the second gate, and the only one that can see a reverse proxy rewriting Host: the server derives the RP from the Host header it received, so it can hand back an rpId that is not a registrable suffix of the page's own origin — the browser's answer to which is a bare `SecurityError`. It compares `availability.rpId` to `location.hostname` and returns the proxy header at fault; `passkeys-self.js` prints it beside the server's own reason, `login.js` and `mobile/auth.js` only withhold the button (an unsigned-in visitor is not told about the install's plumbing). Conditional mediation ("passkey autofill") is deliberately NOT used — it fires a challenge request on every load of the login page, and one office behind a NAT address would burn the ceremony rate limit just by looking at it |
| `public/js/passkeys-self.js` | `PolarisPasskeys` | self-service passkey list / register / rename / remove, and `summary()` for the account-menu row. The totp-self.js arrangement for the same reason: `/auth/passkeys/*` needs no permission, and /users.html is admin-gated |
| `public/js/reservation-notes.js` | `PolarisReservationNotes` | how much of a reservation's notes fit in the FortiGate's 255-character `reserved-address` description once Polaris's `Polaris/<user>: … [<hostname>]` wrapper is paid for (business rule 74). The four IP-panel modals and both mobile reserve/edit sheets render its live counter; it mirrors `reservationNotesBudget` in `src/services/reservationPushService.ts`, which is the half that refuses the save, and `tests/unit/reservationNotesBudgetDom.test.ts` asserts the two agree |
...
| `public/js/asset-merge-modal.js` | `openAssetMergeModal` | the operator merge comparison — survivor pick, per-field winners, dependency-parent winner, the stacked review step. Opened from the Assets page (Sources tab + the two-row bulk bar) AND the duplicate-IP conflict card in the Events page's Conflicts slide-over, which is why it left `assets.js`: `events.html` must not load 21k lines of list machinery for one button. The host page supplies `opts.onMerged` — the modal refreshes nothing itself. Its `_mergeCombineNotes` MIRRORS `combineAssetNotes` in `src/services/assetMergeService.ts` (notes are combined, not won); `tests/unit/mergeNotesMirror.test.ts` evaluates the file in a vm and fails when the two disagree |
| `public/js/pwa-install.js` | `PolarisInstall` | install prompt — **must load first** (`beforeinstallprompt` can fire before boot) |
| `public/js/push.js` + `public/sw.js` | `polarisPush` | Web Push enrollment + service worker (no fetch handler, no Cache Storage). `syncToPreference` reconciles a browser to the account preference at boot and NEVER prompts; `shouldOfferEnrollment` / `recordOfferMade` are the one-time offer put to a browser whose permission has never been asked (rule 39) |
| `public/js/api.js` | — | fetch wrapper, CSRF handling, stale-Secure-cookie pre-check |
| `public/js/app.js` | `THEMES` / `TRANSIT_THEMES` / `isLightTheme` / `_setTheme` / `advanceTheme` | the theme list, the transit palettes the band travels through, the daylight-family test every palette switch reads, and the `themechange` event cached palettes repaint on. A theme owns a position in `THEME_BAND_POS` as well as a token block — adding one without a position strands the band where it stands while painting the new palette, and the phone's `THEME_STRIP_POS` has to gain the same entry |
...
| `public/js/app.js` | `wireModalStepKeys` | the keyboard on a wizard footer — `→`/`←` for Next/Back, and Enter meaning NEXT until the step where Next is gone. One call per wizard (all three opt in); the arbitrations it makes, and the reason Enter must not save a stepped form, are in polaris-ui-canon → canon-modals-wizards.md § Wizard |
...
**Charts** are hand-rolled SVG. They render labeled translucent maintenance bands over window gaps (`_maintenanceBandLayer`), dive to the baseline in red across a failed poll and in **grey** across one the upstream explains (`_CHART_DEP_COLOR`; the same value lives in `public/js/mobile/charts.js` and `src/utils/sparklineSvg.ts` — change one, change all three), and may shade the line by automation severity — the ladder comes from `GET /assets/:id/metric-thresholds` (server-side, off the engine's own `resolveTierLadder`) so shading can never disagree with what actually fires. Two chart shapes beyond the single line have their own canon sections: a **many-series** chart whose series count comes from the data (one line per CPU core — palette rules, legend toggles, and why a missing dimension gets no point rather than a zero) and a **stacked-area** chart for a reading that is a composition rather than a level (the memory bands). **No data series may be red or grey** on any of them, those being the missed-poll and dependency-down colours.
