...
- **One key, three rungs, three distinct verbs — `firmware` is the example.** `read` renders the Repository tab and the asset card's facts (which image is on offer, which login applies); `write` renders upload / delete / make-primary / purge / Set login on the tab; `fullwrite` renders the ONE verb on the asset card, "Upgrade firmware to …", with the card's hint naming the rung when it is withheld. Each surface checks `permAtLeast("firmware", <its rung>)` directly (business rules 43(g), 87).
