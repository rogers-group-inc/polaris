...
- **No filter on the track, and none on the art inside it.** One artwork serves all four palettes here too, and a brightness lift was added and reverted in the same session: the contrast measurement behind it was really describing the desktop dial's rendering bug (see the theme-dial bullet in frontend-shell.md), not the artwork. This strip translates rather than rotates, so it is not the same bug — but nothing has looked at it on a real phone, so it stays unfiltered until something does.
