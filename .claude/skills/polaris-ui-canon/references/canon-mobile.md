...
- **Every theme on the strip has its own `[data-theme="<id>"]` block in mobile.css**, hued from the desktop's tokens. One without a block still renders, on `:root`'s neutral Material greys — which is how nightfall shipped grey on the phone beside the desktop's indigo until 2026-09-23. The daylight token mapping in mobile.css does NOT carry over to a dark theme as written: it sends `--color-bg-secondary` (the darkest ground) to `cont-high`, which inverts every raised surface when "higher" means lighter. The nightfall block keeps `:root`'s lightness ladder and takes only the hues. The phone's chrome colour (`theme-color` in mobile.html, `PolarisTheme.set`, `THEME_COLOR` / `BACKGROUND_COLOR` in `src/api/routes/pwa.ts`) is nightfall's `cont` / `surface`, and those must move together. `tests/unit/themeBandParity.test.ts` fails when a `THEME_STRIP_POS` id has no block that sets `--md-surface`.
