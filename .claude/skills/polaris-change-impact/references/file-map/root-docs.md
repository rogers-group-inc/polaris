...
polaris/
├── CLAUDE.md
├── .claude/skills/                # The eleven project skills (domain model, business rules, API + RBAC, change impact, UI canon, monitoring + discovery, agent, deploy, docs sync, worktree workflow, tech lifecycle). A twelfth, design sync, was retired 2026-09-11 — the bundled /design-sync reads .design-sync/ directly. Each SKILL.md routes to its references/; the four retired root reference docs (architecture, touches, business rules, UI canon) were split into these skills on 2026-09-06. Tracked (the rest of .claude/ is per-developer and gitignored).
...
├── .design-sync/                    # Sources for the Claude Design design-system project at claude.ai/design — the return trip for design/, so future apps designed there inherit the Polaris look. `build.mjs` assembles the gitignored `ds-bundle/` from design/css/ plus `conventions.md` (the class + token vocabulary taught to the design agent) and `cards/` (authored preview cards). `config.json` holds the projectId and is NOT converter config: the /design-sync converter needs a built component library, which this repo has none of, so the upload is assembled off-script. `NOTES.md` is the durable record AND carries the procedure — the trigger table, the upload steps and the gotchas — because /design-sync reads it before acting; a /polaris-design-sync skill held a copy until 2026-09-11 and was retired as a duplicate layer. Tracked.
├── README.md
├── .env.example
├── package.json
├── tsconfig.json
...
│   ├── next-rule-number.mjs         # `npm run rules:next`: the next free business-rule number, derived by scanning main AND every worktree-* branch (invariant lines, `## Rule N` narrative headings, SKILL.md index rows) — never stated in a doc, because a stated number is stale the moment another branch merges (polaris-business-rules → Numbering)
