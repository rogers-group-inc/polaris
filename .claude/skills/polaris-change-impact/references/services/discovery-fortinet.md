...
**When changing this:**
- Adding a scope kind is a three-part change: the `DiscoveryScope` union (now in `discovery/discoveryScope.ts`), a target parameter on that integration’s collector, AND a guarantee the sync layer takes no absence-based destructive action on a one-device result. **Superseded 2026-09-16:** `syncEntraDevices` / `syncActiveDirectoryDevices` DO now sweep on absence (business rule 70, opt-in per integration). They still need no SyncMode, but only because the guarantee is met a different way — `AdDiscoveryResult` / `EntraDiscoveryResult` carry `scoped`, and `adSweepBlockedReason` / `entraSweepBlockedReason` check it FIRST and refuse the pass. That is the cheaper shape when the collector already knows it was scoped; `sweepPhaseEnabled` / `vcenterPassEnabled` remain the pattern when the SYNC layer is the only thing that knows. `syncVcenterDevices` sweeps on absence and carries both guards (`syncArcDevices` isn’t exported and has no absence pass). `sweepPhaseEnabled` in `discoveryEngine.ts` is the pattern to copy.
...
`RunAccumulator.skippedErrorDevices` (a `Set<string>`) names the devices behind `skippedErrorCount`. In memory only and deliberately not a column: its one reader is the `integration.discover.devices_unread` Event that `runDiscovery` writes at the end of the same run.
...
- A `discover.device.complete` for a device already counted as an error **retracts** it — from the Set and the count. `processDevice` logs its first direct-REST failure before deciding whether to re-resolve and retry, so without the retraction a gate that failed once and then succeeded would be reported unread and `done` would exceed `totalDevices`.
...
- proxyQuery is the REST pass-through behind the Query API tool (both the standalone FortiGate integration and FMG's "Directly to FortiGate" mode, via `fortimanagerService → proxyQueryViaFortigate()`). GET is a read; POST/PUT/DELETE reach the live CMDB and the route logs them as `integration.query.write`.
- **fgRequest only maps HTTP statuses to AppError — a request that never got an HTTP answer escapes raw** (undici `TypeError("fetch failed")` with a `cause.code` such as ETIMEDOUT / ECONNREFUSED / a TLS cert code, the AbortError from its own 15 s timeout, or a SyntaxError from a non-JSON 200). That is deliberate for its internal callers: `testConnection` branches on `cause.code`, and probes/push classify the raw error. But the route error handler can only render a raw error as a bare 500 "Internal server error", so **proxyQuery converts them through `describeFgTransportError()` into a 502 "Could not reach FortiGate at <host>:<port> — <why>"**; FMG direct mode appends how the address was chosen (the device's management IP as FortiManager reports it on the configured interface). Convert at the entry point that faces an operator, never inside fgRequest. Pinned by `tests/unit/fgProxyQueryTransportErrors.test.ts`.
...
- These four actions (and the fifth, `asset.fortilink.changed`, written by the controller-link sweep rather than by discovery) are written UNCONDITIONALLY — never through `maybeEmitChangeEvents`/`isChangeActionSubscribed` like the `change.*` family. They're edge-triggered and rare, so a steady fleet writes none; their entries in `notificationTypes.CHANGE_TYPES` only give the automations wizard a picker over an always-present event.
...
- Renaming any of the five `asset.*.changed` action strings must move `CHANGE_TYPE_ACTIONS` in `notificationTypes.ts` in the same commit, or stored automations silently stop matching (`tests/unit/notificationChangeTypes.test.ts` pins the pairing).
...
- **Both mgmt-IP producers missing means the device is DROPPED from the run**, not degraded: `processDevice` logs `discover.device.skip` at error level and returns null, so the gate keeps every field discovery owns (firmware, subnets, leases, switch/AP rosters) while monitoring keeps polling its `Asset.ipAddress` and reporting it up. `resolveDeviceMgmtIp` reads only the interface named by the integration's fleet-wide `mgmtInterface` and `_extractV4` rejects `0.0.0.0` — the normal state of an HA cluster's dedicated mgmt interface (per-member addresses live under `config system ha` → `set ha-mgmt-interfaces`). That is why the warm cache must be keyed on the FMG device name and not `Asset.hostname`; see cross-cutting/fortinet-parent-key-resolution. Prod 2026-09: an 1801F HA pair frozen on old firmware. Never paper over a future instance with a `rawDevice.ip` fallback (can be a public/NAT address) or a silent proxy fallback.
...
- `rpcAttempt` issues its request through `src/utils/tlsDispatcher.ts → tlsFetch()`, never Node's global `fetch`, because `verifySsl:false` attaches an undici dispatcher and a dispatcher is only valid to the undici copy that created it (see the tlsDispatcher entry in `file-map/src-utils-1.md` for the 2026-09-09 `UND_ERR_INVALID_ARG` incident). Tests mock `tlsFetch` rather than the global. Note that a transport fault of this class was classified **retryable**, so each call paid both backoffs before surfacing — a permanent misconfiguration presenting as three slow attempts is a signal to check the transport pairing, not the device.
