...
**What it owns:** CRUD + cached resolver for the editable per-manufacturer telemetry profiles (metric rows, per-metric overrides, custom widgets). A synchronous `getProfileFor` / `listCachedProfiles` pair serves the hot probe path after a boot warm-up.

**The cache is load-bearing since the Phase 4 resolver swap** — these rows are the ONLY source of vendor CPU / memory / temperature / storage OIDs, so a process with a cold cache collects no vendor telemetry at all. Two consequences, both wired in `app.ts → startBackgroundJobs`: the warm runs on **every role** (the seed job that used to be the only warmer is gated on `runsMigrations`, i.e. web/all, so a split-role MONITOR process — the one that walks devices — started empty), and a 30 s interval calls `refreshProfileCacheIfStale()` on every role, because the write paths refresh only the cache of the process that SERVED the write. That leaves every other process stale: the monitor consumers, and behind a load balancer or an HA pair the web replica that did not handle the request. The TTL is 60 s; a failed refresh keeps the previous copy (stale vendor OIDs collect, an empty cache collects nothing) and does not stamp the clock, so the next tick retries.

**Public API:** `MetricKey`, `Aggregate`, `AGGREGATES`, `MetricRowType`, `MetricOverrideRow`, `MetricRow`, `RowReadiness`, `CustomWidgetRow`, `StateProbeSummary`, `ProfileSummary`, `ProfileFull`, `ManufacturerSuggestion`, `ManufacturerSuggestionSource`, `refreshProfileCache`, `refreshProfileCacheIfStale`, `getProfileFor`, `listCachedProfiles`, `updateProfile`, `listProfiles`, `listStateProbes`, `listManufacturerSuggestions`, `mergeManufacturerSuggestions`, `getProfile`, `createProfile`, `updateMetricRow`, `createOverride`, `updateOverride`, `deleteOverride`, `createWidget`, `updateWidget`, `deleteWidget`, `deleteProfile`, `annotateReadiness`, `readinessSummary`, `symbolWarnings`, `symbolWarningsForProfile`, `emitProfileReadinessEvents`, `STD_MIB_KEYS`, `METRIC_KEYS`
...
- **FIVE of the eight metric rows are read at probe time; the other three are descriptive.** `pickDbProfile` in `profileResolver.ts` plucks `model` / `cpu` / `memory` / `temperature` / `storage` and builds the runtime shape from them ALONE — since the Phase 4 swap nothing is layered over the hardcoded entry. (`model` joined the set in that phase, once `parsePattern` / `parseTemplate` made an identity parse expressible as a row.) `interfaces` / `lldp` / `wirelessStations` are deliberately absent — those are table walks with no symbol to swap. This is a hand-maintained claim about code elsewhere, exactly like `pollingCapability.ts`: the profile page renders all seven identically and validates all seven on save, so a row the runtime doesn't read is an operator edit that saves cleanly, reports success, and changes nothing. `storage` was in that state until 2026-08 — it seeded from the hardcoded `disk` block, matched it exactly, and was read by nothing, so nobody could tell. **Adding a row to the plucked set means `pickDbProfile` AND a `*QueryFromMetricPick` translator** (`diskQueryFromMetricPick` is the pattern: the row's combiner is interpreted, and a shape it can't express returns null so the hardcoded baseline survives a half-finished edit rather than being cleared).
...
**Used by:** `src/services/monitoringService.ts` — the four SNMP collectors that read a vendor profile (`collectTelemetrySnmp`, `collectHardwareSensorsViaSnmpSession`, `collectSystemInfoSnmp`, `collectStorageOnlySnmp`), all of which now call `pickDbProfile`. `pickVendorProfileMerged` has NO production caller left: it survives only so `profileResolverParity.test.ts` can keep comparing the two, and goes when the constant does. The two telemetry/temperature collectors build their `ProfileSubject` through `monitoringService → profileSubjectFor()`, which applies the operator's per-stream MIB pin.
...
- **The seed is SMI scaffolding only — nothing under `enterprises`.** Until 2026-09 it carried fifteen vendor anchor arcs and eighteen vendor telemetry leaves so the built-in profiles worked with no upload; Phase 3 of the uniform-SNMP plan removed them (cross-cutting/vendor-snmp-knowledge-boundary.md). A vendor's symbols resolve only once that vendor's MIB is uploaded — leaf AND core module, since FORTINET-FORTIGATE-MIB imports `fortinet` from FORTINET-CORE-MIB — and the readiness surfaces (upload response, profile rows, the boot `manufacturer_profile.unresolved` Event) say exactly what is missing. `oidRegistry.test.ts` pins that no enterprise arc returns; `noEnterpriseOids.test.ts` covers the file. Runbook: `polaris-monitoring-discovery` → runbooks/telemetry-unresolved-symbols.md.
...
**Public API:** `VENDOR_TELEMETRY_PROFILES`, `pickVendorProfile`, `memoryQueryToDoubleScalar`, `VendorTelemetryProfile`, `CpuQuery`, `MemoryQuery`, `DiskQuery`, `TemperatureQuery`, `ModelQuery` (symbol + parse fn + `rowParse`, the SAME rule in row shape — a function cannot be seeded, so without it the `model` metric row is created empty on a fresh install and the identity query exists only in code; FortiSwitch `fsSysVersion` → `utils/fortiswitchModel.ts` — consumed by `collectSystemInfoSnmp`, adopted onto `Asset.model` by `recordSystemInfoResult` only while the stored model is empty/generic; since Phase 4 this IS part of the editable ManufacturerProfile surface: the `model` metric row carries the symbol plus a declarative `parsePattern` / `parseTemplate`, and `pickDbProfile` builds `ModelQuery.parse` from them via `utils/modelParse.applyModelParse`. The hardcoded `parse` FUNCTION here is now seed material only). `memoryQueryToDoubleScalar(mem)` translates a hardcoded `MemoryQuery` into the editable Manufacturer Profile's double-scalar shape (`{type, symbol, symbolB, transform}` with the matching `CombinerKind`) — consumed by `seedManufacturerProfiles` and `backfillManufacturerProfileMemoryComposition`. Returns null for empty memory blocks.
...
**Invariants:**
- `match` regex is tested against `"${manufacturer ?? ''} ${os ?? ''} ${model ?? ''}".trim()` (all three fields optional). **Its `.source` is also what `seedManufacturerProfiles` stamps as the profile's `matchPattern`**, which is why the migration's six literal UPDATEs and the seed agree without the list being typed twice — editing a `match` here changes what a fresh install's "also applies when" says.
