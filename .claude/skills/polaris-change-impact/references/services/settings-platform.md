...
- **Two Event actions, not one.** `resetEventSchema` accepts only `actionPattern` and `resourceType` (no `detailsMatch`), so a single-action design would have the seeded automation's event-mode reset match its own escalation and self-clear immediately. Emitting `platform.lifecycle_recovered` costs nothing and lets the automation genuinely clear when the operator finishes the upgrade. `capacityService` splits its verb the same way and for the same reason (`CAPACITY_CHANGED_ACTION` / `CAPACITY_RECOVERED_ACTION`, 2026-09-17) — it is the rule this entry used to cite as the one that settled for a timer. Both pairs write the recovery verb ONLY on a landing at the healthy state, so a partial recovery does not clear the alert, and both keep their two halves on the same subject or the clear misses.
...
- **Host facts must not hardcode a PostgreSQL major.** `probeHostFacts` reads the Timescale version with `rpm -qa timescaledb-2-postgresql-*`, not `-q` against one major: the package name carries the major, so a literal reports "not installed" on every host that has moved. That figure is compared BETWEEN the two nodes to catch a skew that would stop a replica replaying the primary WAL, so a silent "none" on both sides reads as agreement — the worst available answer. A null reading means **"nothing to compare"**, never "no TimescaleDB on this host" — the extension is required (business rule 52), so the only way the probe's catch fires is `rpm` itself being unavailable. Note the null is not inert: `haEnrollmentService.ts` omits `shared_preload_libraries: timescaledb` from the generated standby Patroni config when it is absent. `enableHa` still carries `/usr/pgsql-17/bin` and `/var/lib/pgsql/17/data` as fallbacks for a host that was never probed; keep them in step with `PG_MAJOR` in `deploy/setup-rhel.sh` and `deploy/ha/setup-rhel-ha.sh`.
...
- **A database node's bundle refuses to build without `POLARIS_PROXY_CERT_PATH`, and refuses FIRST.** The standby must serve the identical leaf agents pin, so a bundle without it would build a node the whole fleet rejects. The check sits with the other 409s at the top of `buildNodeBundle`, above `issueEtcdCert` — not down in `readProxyCert` where the value is actually used. It was there until 2026-09-10, and being there meant it almost never fired: the `entries.push(...)` for a non-witness role evaluates `readEnvFile()` before `readProxyCert()` (arguments evaluate left to right), so on any host whose `.env` was unreadable the operator got a 500 about `.env` and could never reach the one message that names what they have to fix. It also minted a node certificate first and threw it away. Keep new preconditions up there for the same two reasons.

- **The teardown script follows THIS host, not the shipped default.** `renderTeardownScript` derives the PostgreSQL unit to unmask, enable and start from the major in `hostFacts.pgBinDir` / `pgdata` (`pgMajorFromPaths`), falling back to `DEFAULT_PG_MAJOR` only when neither was ever probed. A cluster built on 15 is torn down to `postgresql-15` even after the setup scripts moved to 17 — naming the wrong unit leaves the operator with a database that never starts and no clue why. Two unit tests pin both directions.
...
**What it owns:** The active-instance heartbeat and the WAL-rate ring behind the active/standby HA deployment (docs/HA.md). Two Setting rows: `ha.activeInstance` = `{instanceId, hostname, pid, at}`, re-stamped every 30s by the scheduler role and deleted on a clean shutdown, and `ha.walSamples` = a 288-entry (24h, one per 5 min) ring of `pg_current_wal_lsn()` readings used to size the WAN link, `max_slot_wal_keep_size` and `maximum_lag_on_failover` BEFORE anyone enables HA. It also owns the install's identity file, `<STATE_DIR>/data/instance-id`.

**Public API:** `evaluateHeartbeat` (pure), `checkActiveInstanceConflict`, `stampActiveInstance`, `releaseActiveInstance`, `readActiveInstance`, `resolveInstanceId`, `getSelfIdentity`, `appendWalSample`, `isHeartbeatEnabled`, `__resetInstanceIdentityForTests`, `ACTIVE_INSTANCE_KEY`, `WAL_SAMPLES_KEY`, `INSTANCE_ID_FILE`, `HEARTBEAT_INTERVAL_MS`, `CONFLICT_WINDOW_MS`, `WAL_SAMPLE_EVERY_TICKS`, `WAL_SAMPLE_RING_SIZE`, `ActiveInstanceStamp`, `InstanceIdentity`, `WalSample`, `HeartbeatVerdict`.

**Cross-service deps:** `prisma` (settings), `src/utils/dbConnections.ts` (`getDirectDatabaseUrl` gates the WAL read), `src/utils/paths.ts` (`STATE_DIR` locates the identity file), `node:os`, `node:fs`, `node:crypto`. No other service.

**Used by:** `src/jobs/activeInstanceHeartbeat.ts` (the 30s tick, scheduler role only), `src/app.ts` (boot guard before `listen()`, gated on `cfg.runsSchedulers`, which increments `polaris_ha_active_instance_conflict_total` and exits 1 on a conflict; and the SIGTERM/SIGINT handler, which calls `releaseActiveInstance()` on the same role gate).

**Invariants:**
- **Identity is the INSTALL, not `os.hostname()`** (business rule 62). `resolveInstanceId()` returns `POLARIS_HA_INSTANCE_ID`, else a uuid persisted at `INSTANCE_ID_FILE` (`<STATE_DIR>/data/instance-id`), else — only when that file cannot be written — the hostname, with a warning. This is not cosmetic: a container's hostname is its container id, regenerated on every recreate, so the hostname rule made every Docker image upgrade look like a second host and refuse to boot. Never reintroduce a hostname comparison as the primary identity. The known cost is that copying a whole state directory to a second host clones the id; docs/HA.md §5 tells the operator to set `POLARIS_HA_INSTANCE_ID` there.
- **A stamp with no `instanceId` falls back to hostname comparison.** Those were written by a release predating the field; guessing "that's me" would open exactly the hole the guard closes. It costs one 90s wait on the upgrade that introduces the id, and nothing after.
- **The asymmetry is the whole design.** A FOREIGN stamp younger than `CONFLICT_WINDOW_MS` blocks a boot; this install's OWN stamp never does, whatever its age. Without that exception a systemd restart mid-tick would leave the app permanently refusing to start.
- **A clean shutdown releases the claim, a kill does not.** `releaseActiveInstance()` deletes the row only when the stored stamp is ours (never a peer's) and latches `released` so an in-flight tick cannot resurrect it; `stampActiveInstance()` returns null afterwards. `kill -9`, an OOM or a host reset leave the stamp, which is precisely when the 90s window should apply.
...
**When changing this:** the window arithmetic is asserted in `tests/unit/haHeartbeatService.test.ts` (`CONFLICT_WINDOW_MS / HEARTBEAT_INTERVAL_MS === 3`, ring = 24h) — change the constants and those assertions together, and update docs/HA.md §5, which states the 30s/90s numbers to operators. That file also pins the container-recreate case (predecessor's fresh stamp, new hostname, same id ⇒ no conflict); keep it, it is the regression the identity change exists for. Any change to how identity is derived is an operator-visible change in three more places: `.env.example`, `polaris-deploy/references/env-vars.md` and the Docker section of docs/INSTALL.md. If the stamp ever gains a reader beyond the boot guard (an HA status card, for example), state whether that reader tolerates a stale row before relying on it.
...
- **Six queue names (Phase 2):** `polaris-monitor-probe`, `polaris-monitor-fastfiltered`, `polaris-monitor-telemetry`, `polaris-monitor-systeminfo`, `polaris-monitor-lldp`, `polaris-monitor-storage` (jobs prefixed `polaris-monitor-*`). LLDP and Storage each get their own dedicated worker pool (default 12 workers, env `POLARIS_MONITOR_LLDP_WORKERS` / `POLARIS_MONITOR_STORAGE_WORKERS`) running `runLldpFor` / `runStorageFor` from monitoringService. Floating priority order at Phase 2: probe > fastFiltered > lldp > storage > telemetry > systemInfo (current order below). The publisher in `monitorAssets.ts` gates LLDP/Storage on `Asset.lastLldpAt + lldpIntervalSeconds` / `Asset.lastStorageAt + storageIntervalSeconds`; the legacy `collectSystemInfo` still walks both as session-coalesced side effects on the same SNMP session and the persist paths are idempotent against double-walks. **SD-WAN (2026-09-23)** adds `polaris-monitor-sdwan` (`QUEUE_NAMES.sdwan`, `expireInSeconds` 150 — two small REST GETs under `systemInfoTimeoutMs`), dedicated pool `POLARIS_MONITOR_SDWAN_WORKERS` (default 8, slot prefix `sdwan` in `utils/workerSlotPool.ts`) running `runSdwanFor`; published from the LIGHT tick, and `FLOAT_PRIORITY` is now probe > fastFiltered > sdwan > lldp > storage > processes > eventLog > telemetry > systemInfo (sdwan sits with fastFiltered: a per-minute stream whose value is timeliness).
...
- **The pull step discards local changes to tracked files before it pulls (since 2026-09-10).** `discardLocalChanges()` runs `git status --porcelain --untracked-files=no` and, when anything is listed, `git reset --hard HEAD`; the paths land on the step message ("discarded local changes to N tracked files; …") and in the `server.update.failed` / `server.update.applied` Event as `discardedLocalChanges`. The install checkout is not an editing surface — `.env`, `data/` and the app user's caches are untracked and never touched — but it CAN be dirty: the fallback scripts' rollback used `git checkout <old> -- .`, which restores the old content and leaves HEAD at the new commit, so after a failed `update-linux.sh --force` prod read as 77 locally-modified files and `git pull --ff-only` refused ("Your local changes … would be overwritten by merge"). Both scripts now `git reset --hard "$OLD_COMMIT"` (pinned by `tests/unit/updateScriptsContract.test.ts`); the in-app reset is the belt for installs that rolled back on older script code. It subsumes the older `git checkout -- package-lock.json` special case. Pinned by `tests/unit/updatePipeline.test.ts` ("discards local changes to tracked files before pulling").
- **The registry-preflight refusal message states what THIS process knows about `NODE_EXTRA_CA_CERTS`** (`nodeExtraCaNote()`): unset → "NOT set in this process's environment … `systemctl restart polaris.target`" (systemd reads `.env` only at unit start, and the npm child inherits this process's environment — there is no npm process to restart); set to a missing file → says so with the path; set to a real file → "the variable is not the problem", the bundle lacks the CA. Prod 2026-09-10 had the line in `.env` and still failed here; which of the three it was is the whole diagnosis. Only the fast-refusal branch carries it (a hang is not a certificate problem). `docs/INSTALL.md` → "Still failing with the line in `.env`?" mirrors the three sentences.
...
- **Update source repo is configurable.** `ensureUpdateRemote()` runs before the fetch in `checkForUpdates()` AND before the pull in `applyUpdate()`. When `POLARIS_UPDATE_REPO` (env) is SET it repoints the `origin` remote at that URL (idempotent — only rewrites when the URL differs; `git remote add`s if origin is missing; non-fatal). When UNSET it's a no-op — the install's existing `origin` is left as-is (updates come from wherever it was cloned). `getUpdateRepoInfo()` reports the active repo + source (`"env"` vs `"origin"`) and is exposed at `GET /server-settings/updates/repo` for the Application Updates card's "Update source" row. The fallback script (`deploy/update-linux.sh`) reads the same `.env` var and repoints origin only when set, in lockstep — keep both in sync when changing the env-var name or the set/unset semantics.
- **The unit sync migrates the PostgreSQL dependency into a drop-in before it overwrites a unit (since 2026-09-09).** The shipped units named `Requires=postgresql-15.service` inline; they now name no PostgreSQL unit, because the major is a HOST fact and `cp -f` re-asserts whatever is in `deploy/` onto every host at every update. So `restartService()` reads the installed unit first, and when it still carries an inline `Requires=postgresql*.service` writes that name to `<unit>.d/20-postgres.conf` — never over an existing drop-in (an operator-set name wins), and never on an HA node (`/etc/polaris/ha-node` or a `10-ha.conf`, whose `After=`/`Requires=` reset points at `patroni.service`; a later-sorting `20-` file would re-add postgres after that reset and race Patroni for the data directory). Lockstep with `preserve_postgres_dependency()` in `deploy/update-linux.sh` — the same block, in the language of each path. Without it the first update after the cutover silently drops the ordering and `polaris-migrate` races PostgreSQL at boot, which takes the whole target down.
...
- **`restartService()` is not only a restart — it is the only path that pushes shipped deploy artifacts onto an existing install.** The transient `systemd-run` unit it spawns syncs three sets before it kicks `polaris.target`: the shipped `deploy/polaris-*.service` + `polaris.target` unit files (install-if-missing, so a unit shipping for the first time lands on upgraded hosts); in proxy mode the rendered nginx config, validated with `nginx -t` and reloaded FIRST so new location blocks are live before the app returns; and, on a host with the `/etc/polaris/ha-node` marker, the HA artifacts that live outside the tree once installed (`/usr/local/sbin/polaris-ha-role`, the HA unit + timer, `patroni.service.d/10-polaris.conf`, the five `polaris-*.service.d/10-ha.conf` drop-ins) — those **refresh-only-if-present**, because `setup-rhel-ha.sh` decides which of them a node's role gets. Every one of the three is mirrored in `deploy/update-linux.sh` for the manual path and drifts silently if only one side is edited; the register is in `polaris-deploy` → cross-cutting-deployment.md.

**When changing this:**
- Test update path on both git-backed and Docker installs; verify "disabled" message is clear.
- Adding a shipped deploy artifact means editing the sync list in BOTH `restartService()` and `deploy/update-linux.sh`. An artifact installed outside `/opt/polaris` reaches a host through nothing else — the tree it lives in is not the copy that runs.
...
- **`sslmode` is translated into libpq's vocabulary on the way, never copied (rule 51).** `DATABASE_URL` is a node-postgres URL; the wizard writes `sslmode=no-verify` for "Allow self-signed certificate" and libpq has no such value, so a verbatim copy killed `pg_dump` (and `psql`, hence restore) instantly on every wizard-created install with TLS to the database — manual, scheduled AND pre-update backups, all behind "see the server log" (2026-09-11). `utils/pgEnv.ts → libpqSslMode()` owns the mapping. The scripted RHEL/Ubuntu path never reproduced it: `deploy/update-linux.sh` dumps over a unix socket as the `postgres` OS user and builds no URL, so this is a remote-DB / container failure that a green scripted install and a plaintext CI service container are both blind to.
