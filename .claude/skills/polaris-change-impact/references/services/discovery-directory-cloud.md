...
- The serial bridge (`src/utils/hardwareIdentity.ts`) is what lets an Arc machine merge onto an AD-discovered asset — those two sources share NO definitive key otherwise. Matching on a raw serial is unsafe: `normalizeHardwareSerial` rejects vendor placeholders, and `indexUniqueBy` must ALSO drop serials claimed by two assets. The motivating case was our own agent — its Windows collector reported `SystemSKU` (a model SKU) because the registry publishes no serial at all — and **agent 0.20.1 fixed that end** by reading the raw SMBIOS table (`agent/internal/collectors/smbios.go` → `readSMBIOSDMI()`). The uniqueness guard is NOT retired with it: an asset stamped by an older agent keeps the SKU until that agent upgrades and reports again, cloned VMs duplicate a template's serial, and a placeholder the list has not met behaves identically. Never match on a serial without both guards; the failure mode is merging a whole model line into one asset, which is silent.
