...
- **`DiscoveredVcenterVm.guestIdentityRead` records that the guest ANSWERED, and is not derivable from anything else here.** `guestIp` / `guestHostname` / `guestOsFullName` are only attempted when `toolsRunState === "RUNNING"`, and the identity call is per-call try/caught, so a null address conflates Tools-off, an identity call that threw, and a guest that genuinely reports no address. Discovery needs those apart to decide whether it may REMOVE a stored address (business rule 81), so the flag is set immediately after the request returns and BEFORE its fields are read — what it asserts is that the guest replied, whatever it replied with. `toolsRunState` is NOT a substitute: it comes from a separate `/tools` request that can succeed while `/guest/identity` fails. Any new early-return or swallowed failure around the guest read has to leave the flag false, exactly as `inventoryComplete` works for the sweep.
...
- **The PerformanceManager is a separate surface with three rules of its own.** (1) **Real-time only.** `intervalId` is pinned to 20 because per-INSTANCE series — the individual cores — are collected at the real-time interval regardless of configuration, while the historical rollups are gated on the vCenter statistics level (1 on a default install, aggregate only). Query a historical interval and you get one series on most installs, which reads as a code bug. (2) **Counter ids are per-vCenter**, so `cpu.usage.average` is resolved by group+name+rollup out of `QueryPerfCounterByLevel` and memoized for 6h keyed by `host:port` — the table is ~150 KB and immutable for a vCenter build, so re-fetching it beside every 30-second cache refresh would be the most expensive thing in the file. (3) **It degrades to nulls on its own**, like every other SOAP surface here: a service account without the Performance privilege is the common real-world case, and `attachPerCoreCpu` swallows the failure rather than costing the caller CPU, memory, power state and the whole interface table. The AGGREGATE series (empty instance) is discarded — quickStats already has a better one computed against the entity's real clock rate, and two aggregates a rounding step apart read as the average line having lost its own cores. Chunked at 200 entities and run SEQUENTIALLY: a vCenter is one appliance, and every other fetch here is serial against it.
- **A VM's memory bands cost nothing; a host's cost a perf query.** The five VM bands are scalars on the quickStats property set already being fetched, so they ride it. `HostListSummaryQuickStats` publishes only `overallMemoryUsage`, so a host's balloon and swap come from the perf pass — which is why `fetchVcenterPerfSamples` takes `wantMemory` and only the host caller sets it. Asking perf for a VM's balloon would be a second answer to a question quickStats already answered.
...
- Changing quickStats or host-stats fields → update `readVcenterAsset` / `collectTelemetryVcenter` / `buildVcenterSystemInfo` (monitoringService) + the parser fixtures in tests/unit/vcenterService.test.ts. A new TELEMETRY column is a longer lockstep: `TelemetrySample` + `collectTelemetryVcenter` + `recordTelemetryResult` (monitoringService), `TelemetrySampleRow` (sampleWriteBuffer), the agent push path's explicit nulls (routes/agents.ts), `TelemetryHistoryRow` + both tier branches (sampleHistoryService), both rollup statements (sampleRollupService), the schema + a migration, and the chart's band table (public/js/assets.js). Adding a STREAM the method serves is a five-place lockstep: `VCENTER_STREAMS` (pollingCompatibility.ts), `defaultPollingForSource` (monitoringService), the collector, the `assets.ts` PUT guard, and the `_VCENTER_STREAMS` + `_polarisSourceDefaultPolling` mirrors in public/js/integrations.js.
