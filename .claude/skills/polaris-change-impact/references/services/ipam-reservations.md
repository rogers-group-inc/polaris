...
**Public API:** pure — `claimIsOperatorOwned(row)`, `claimIsCurrent(row, cutoff)`, `distinctDeviceCount(members)`, `groupCurrentClaims(rows, cutoff, ctx?)`, `memberSetKey(members)`, `pickPrimaryMemberId(members)`, `groupHasEligibleType(members)`, `addressIsDeliberate(reservation)`, `groupHasDisjointSources(members, sourceKindsByAsset)`, `resolveMergeTargets(members, survivorId, rawAbsorbIds)`, `toStoredMember(row)`, `duplicateIpRejectMessage(conflict)`; DB — `loadDuplicateIpClaims()`, `loadCrossSourceContext(rows)`, `reconcileDuplicateIpConflicts()` (the job's entry point), `reassignDuplicateIpAsset(conflict, assetId, rawIp, actor)`, `mergeDuplicateIpAssets(conflict, survivorAssetId, absorbIds, actor)`, `logDuplicateIpDismissal(conflict, actor)`, `logScanFailure(err)`. Constants `DUPLICATE_IP_COLLISION_REASON`, `CLAIM_FRESH_DAYS`, `CONFLICT_ELIGIBLE_ASSET_TYPES`, `DELIBERATE_RESERVATION_SOURCE_TYPES`, `EMPTY_CROSS_SOURCE_CONTEXT`. **Write-time check (rule 40(i), 2026-09-22):** `reconcileDuplicateIpConflicts(scope?)` now takes an optional `{ ips }` scope; `repointDuplicateIpConflicts(fromAssetId, toAssetId)` moves pending duplicate-ip cards off an asset a merge is about to delete (the cascade would otherwise drop them unaudited — `mergeDuplicateIpAssets` does the same for its own row); `reconcileDuplicateIpForAddresses(ips)` is the scoped call the asset routes make and returns the open conflict per address (`WriteTimeIpConflict`: `conflictId`, `ip`, `qualifiedBy`, `members`); `DuplicateIpReconcileResult.conflicts` carries the same for every pass. `checkIpForIncomingClaim(incoming)` (DB) and `simulateIncomingClaim(rows, incoming, cutoff, ctx?, now?)` (pure) answer the form's pre-save question by grouping a synthetic operator-owned row (`INCOMING_CLAIM_ID`) with the real claims; `IpCheckResult` / `IpHolder` / `IncomingClaim` are the shapes. `evaluateDuplicateIp(ip)` exports the single-address evaluator the reassign verb always used.

**Cross-service deps:** `prisma`, `logEvent` + `buildChanges` (eventLogService), `UNMONITORABLE_STATUSES` (utils/assetInvariants), `isValidIpAddress` (utils/cidr), `resolvePendingIpOverrideConflicts` (ipOverrideService), `mergeAssets` (assetMergeService — the merge verb delegates rather than re-implementing an absorb), `buildIpContexts` (subnetService — subnet containment for the cross-source clause's reservation lookup, the same single implementation `ipUpstreamChainService` uses).

**Used by:** `src/jobs/detectDuplicateIpAssets.ts` (10-min sweep, scheduler role), `src/api/routes/assets.ts` (`GET /ip-check` → `checkIpForIncomingClaim`; `POST /` and `PUT /:id` → `reconcileDuplicateIpForAddresses` on the address written / moved, awaited, returned as `ipConflict`; `POST /:id/merge` → `repointDuplicateIpConflicts` before the merge, then the same on the survivor's address AND the absorbed asset's old one, awaited), `src/services/duplicateSerialConflictService.ts` (`absorbDuplicateSerialGroup`, the automatic serial merge — same repoint + reconcile), `src/api/routes/conflicts.ts` (`POST /:id/reassign-ip` + the duplicate-IP branch of `POST /:id/merge`), `src/services/conflictResolutionService.ts` (imports `DUPLICATE_IP_COLLISION_REASON` + `logDuplicateIpDismissal` for the accept-refusal and reject branches). Frontend reader: `renderDuplicateIpConflictCard` in `public/js/events.js` + the reason label in `public/js/widgets/conflictQueue.js`.
...
- A group qualifies through ONE of THREE clauses, and `DuplicateIpGroup.qualifiedBy` records which (`"asset-type"` | `"cross-source"` | `"operator-addressed"`), carried onto `proposedAssetFields.qualifiedBy` so the card can pick its explainer. Precedence when several apply: asset-type, then cross-source, then operator-addressed (oldest and broadest first).
- **(i) operator-addressed** — at least one CURRENT claimant is operator-owned (`claimIsOperatorOwned`: `ipSource="manual"` or a pin equal to the address). A typed address is a chosen address whatever the device type. Still needs two devices (c) and a current counterpart (b) — an operator claim is current by definition, so it can never be the stale side. The SQL prefilter admits it as a third `bool_or`.
- **A scoped reconcile neither reads nor closes pending conflicts outside its addresses.** `reconcileDuplicateIpConflicts({ ips })` loads claims for those addresses only (`loadClaimsForIps`, no "at least two" and no eligibility pre-filter — the JS decides) and filters the pending set to them BEFORE the stranded-row and auto-close passes. Drop that filter and a save on one address auto-closes every real conflict on every other.
- **The pre-save simulation and the save's reconcile share `groupCurrentClaims`.** `simulateIncomingClaim` adds one synthetic row (`INCOMING_CLAIM_ID`, `ipSource="manual"`, current) to the real claims and asks the same function the reconcile asks. That is the whole guarantee that the dialog's warning, the raised card and the sweep's steady state agree; a second qualification path here would let them drift.
- **(g) asset-type** — at least ONE claimant whose `assetType` is in `CONFLICT_ELIGIBLE_ASSET_TYPES` (switch / access_point / firewall / server), with every other claimant still in `members[]`, since an endpoint that took an AP's address is what the card has to name. Tested on the CURRENT claims, so a stale switch record can't license an endpoint-only conflict. Checked FIRST and wins the label when both clauses apply.
- **(h) cross-source** — `groupHasDisjointSources` (two claimants whose `AssetSource.sourceKind` sets share nothing; a member with NO source rows abstains rather than qualifying) AND `addressIsDeliberate` for the address. This is the one-device-two-integrations case, whose answer is a merge rather than a renumber.
- `addressIsDeliberate` reads all THREE reservation columns (business rule 23): `vipInfo` non-null, `dhcpBinding === "reservation"`, or `sourceType` in `DELIBERATE_RESERVATION_SOURCE_TYPES`. Reading `sourceType` alone is wrong in both directions — discovery Phase 3c stamps `vipInfo` on a row still labelled `dhcp_lease`, and Phase 5 writes `dhcpBinding` without ever flipping `sourceType`. A plain `dhcp_lease` is deliberately NOT deliberate: two devices may legitimately trade a leased address inside the freshness window.
- The scan's SQL prefilters on BOTH clauses — `bool_or` over `CONFLICT_ELIGIBLE_ASSET_TYPES`, OR a join against the `deliberate` CTE — and is a SUPERSET of each (it cannot see the freshness verdict, the source sets, or subnet containment). `groupHasEligibleType` / `groupHasDisjointSources` + `addressIsDeliberate` in JS are the decisions. Keep them in step — the SQL reads the same exported constants, so don't inline either list into the query.
- `loadCrossSourceContext` resolves the clause's two inputs and MUST be called by every caller of `groupCurrentClaims` that can raise or close (both `reconcileDuplicateIpConflicts` and `evaluateIp` do). Without it the refresh pass re-derives a cross-source group as ineligible and auto-closes the conflict the scan just raised, and the two fight every tick. The default `EMPTY_CROSS_SOURCE_CONTEXT` falls back to the asset-type clause alone, which is what keeps the pure function testable with no database.
- The reservation lookup goes through `buildIpContexts` (containment, most-specific subnet wins) rather than matching `Reservation.ipAddress` alone — a reservation in an unrelated overlapping block must not license a conflict.
...
- The card's three action wirings (`[data-dupip-apply]`, `[data-dupip-merge]` and `[data-dupip-review]`) are bound in `loadConflicts` in events.js, not in the renderer — a new action needs both halves. `[data-dupip-review]` opens the shared merge modal (`public/js/asset-merge-modal.js`) rather than calling a conflict endpoint, so it resolves the ASSETS and lets the next sweep auto-close the card.
- `qualifiedBy` is absent on conflicts raised before the cross-source clause shipped; the card treats a missing value as `"asset-type"`. Don't make it required without a backfill. A new value needs a lead sentence in `renderDuplicateIpConflictCard` (events.js) or the card falls through to the generic asset-type explainer.
- The write-time triggers are the two operator IP writes (`POST /assets`, `PUT /assets/:id` on a CHANGED address) plus `POST /assets/:id/merge` (and the automatic serial merge). A merge must call `repointDuplicateIpConflicts` BEFORE `mergeAssets` and reconcile BOTH addresses after — the survivor's and the absorbed asset's old one, since a survivor that kept its own address leaves the card on the absorbed row's. A new route that stages `ipAddress` by operator hand should call `reconcileDuplicateIpForAddresses` on the addresses it touched (old and new on a move) and, if it has a form, `GET /assets/ip-check` before the write. Discovery-origin writes stay the sweep's business — do NOT hook them (rule 40's "a sweep, not a write-time hook" still holds for the seventeen projection sites).
- Merge is `assets:fullwrite` on every path (`POST /assets/:id/merge`, the conflict verb's chained check, `openAssetMergeModal`, the card's merge verbs via `canMerge = permAtLeast("assets","fullwrite")`, and `GET /assets/ip-check`'s `canMerge`). Change the level in all six places or the UI and API disagree again.
...
- Adding a `ReservationSourceType` value means deciding whether it belongs in `DELIBERATE_RESERVATION_SOURCE_TYPES` (business rule 40(h)). The question is not "is it discovered" but "did somebody choose this address" — a pool hand-out is the only thing that must stay out.
...
- **Device-owned rows are read-only, and the guard lives in the ROUTE, not here.** A `vip` / `interface_ip` reservation cannot be edited or released from Polaris (`assertNotDeviceOwned` in `api/routes/reservations.ts`, 409). It is deliberately NOT a service-layer check, because discovery's own reconcile calls `releaseReservation` on exactly these rows when a VIP disappears from the gate — moving it down here would strand every retired VIP as an active reservation forever.
- **But a VIP address may be CLAIMED** (business rule 77). `vip` is in `isSupersedableByCreate`, so a create at its address supersedes it instead of 409-ing; `releaseSupersededDhcpLeaseAt` releases it with no device I/O (no push pointers, not `dhcp_lease`, so neither unpush branch nor the lease-expiry branch fires) and RETURNS its `vipInfo`, which `persistReservationRow` stamps onto the new row — the address keeps reporting its VIP and the panel reads "VIP / Reserved". `interface_ip` is still refused: that address is live on an interface. The two types part company here and nowhere else, which is why `DEVICE_OWNED_SOURCE_TYPES` (edit/release refusal, and business rule 41's chassis diff) still holds both.
...
- **On a push-eligible subnet the notes are the device's description, so they are budgeted at save time** (business rule 74). `createReservationFlow` calls `assertReservationDescriptionFits` in phase 3 — beside the MAC and deviceName checks, before the row exists — and `updateReservation` calls it before the MAC branch, so neither refusal has touched a device. The edit judges what the update will STORE (`undefined` means "not changing that field") and only when it TOUCHES `hostname` or `notes`: a row whose note predates the rule, or came from discovery, must stay editable, or every save on it 400s and the row is stranded. Nothing off a push-eligible subnet is judged — `notes` is `@db.Text` with no device field behind it
- `listReservations` decorates every row with `pushEligible: boolean` (true when integration is fortimanager/fortigate AND `config.pushReservations === true` AND `ipAddress` is non-null) and strips the raw integration config from the response — callers only need the flag and config can carry credentials. Nothing on the phone reads it from this list any more — the Reservations tab that did was retired 2026-09-23; the mobile network sheet reads the SUBNET-level `pushEligible` from `/subnets/:id/ips` instead.
...
**What it owns:** Per-subnet "discover from device" reconciler — the action behind the **Discover** button in the IP panel slide-in (the button read **Refresh** until 2026-09-21; the route, this service, its Event action and this entry's function name keep that spelling because they are citation keys). Queries the originating FortiGate for ONE DHCP scope (CMDB reservations + live leases) AND the gate's whole `firewall/vip` table, reconciles both against Polaris's rows on the same subnet, and bumps `Subnet.lastDiscoveredAt`. Manual / interface-IP rows are left alone; `vip` rows are NOT (business rule 77 — see the invariants).

**Public API:** refreshSubnet(subnetId, actor) → { lastDiscoveredAt, created, updated, released, skipped, vipsStamped, vipsCleared, vipError }.

**Cross-service deps:** reservationPushService (buildTransportForIntegration, findScopeIdForCidr, listReservedAddresses, callFortiOs, normalizeMac), utils/vipAddressFacts (parseVipRow, vipIpRoles, vipInfoSnapshot, vipInfoDiffers, decideVipDhcpBinding), utils/cidr (ipInCidr), events.logEvent.
...
- Manual / interface-IP rows on the same subnet are skipped — the next full integration discovery is where Polaris raises hostname/owner conflicts on those rows via upsertConflict.
- **A `vip` row is not skipped: it LEARNS the DHCP fact** via `decideVipDhcpBinding`, which writes `dhcpBinding` and a fill-only `macAddress` and never `sourceType` or `expiresAt` (business rules 23 and 77). This is what puts "VIP / Leased" in the panel's Status column.
- **The VIP read is settled, never awaited alongside the DHCP reads.** A gate that answers for DHCP and refuses `firewall/vip` (an API token scoped away from the firewall CMDB) still completes its DHCP reconcile; the failure rides back as `vipError` and EVERY VIP decision is skipped for the pass. "Could not read" must never collapse into "there are none" (business rule 53) — that would retire every VIP fact on the subnet.
- **VIP retirement is scoped to snapshots naming THIS gate** (`vipInfo.device === subnet.fortigateDevice`). RFC1918 space repeats behind different FortiGates, the same reason business rule 17 scopes ARP evidence per device.
- A retired VIP takes one of three paths: a non-`vip` row keeps its own sourceType and only loses `vipInfo`; a `vip` row with a DHCP entry at the same address converts in place (the narrow form of discovery Phase 5's succession, one `reservation.vip.replaced` Event); a `vip` row nothing else claims is released (`reservation.vip.released`).
- The VIP pass re-reads the subnet's active rows after the DHCP flush, so a row created by the SAME pass can receive its VIP snapshot without a second click.
...
- If the read shape from FortiOS `/api/v2/monitor/system/dhcp` changes, update both `fetchLiveLeasesForScope` here AND the corresponding shape in fortimanagerService.ts / fortigateService.ts so the partial discover and full discovery stay in sync.
- The VIP read goes through `utils/vipAddressFacts.ts → parseVipRow()`, which accepts all three encodings a `firewall/vip` row arrives in. Never hand-parse the row here — the two full-discovery readers in fortimanagerService.ts and fortigateService.ts each handle a different subset, and that divergence is what dropped proxy-mode mapped IPs once already.
- Clearing `vipInfo` needs `Prisma.DbNull`, not a JS `null`.
...
**Public API:** listSubnets, getSubnet, createSubnet, allocateNextSubnet, bulkAllocate, previewBulkAllocate, resolveBlockForCidr (most specific containing block, same IP version, via `utils/cidr.ts → mostSpecificContaining()`), updateSubnet, listMoveTargets + moveSubnet (re-parent onto another block), getSubnetIps, deleteSubnet, buildIpContexts + IpContext (batched IP → most-specific containing subnet + active-reservation summary; THE single implementation of the `cidr >>= ip` / `masklen DESC` containment SQL).
...
**Used by:** src/api/routes/subnets.ts (all operations), src/api/routes/assets.ts (buildIpContexts — per-row `ipContext` for the asset table's View Lease button), src/services/dnsResolvedReservationService.ts (buildIpContexts — target-subnet resolution in the reconciler), src/services/ipContextService.ts (buildIpContexts — the containing network behind the Add Asset IP cross-reference; deliberately reuses this rather than re-implementing the `cidr >>= ip` / `masklen DESC` query), src/services/reservationService.ts (subnet lookups, status checks), src/services/utilizationService.ts (subnet status grouping). Browser-side consumers of `listSubnets`' rows: public/js/subnets.js (the IPAM → Networks list, its row menu's delete/archive confirmations and both exports) and public/js/block-panel.js (the block drill-in panel's network table).

**Invariants:**
- **`listSubnets` counts reservations TWICE, and the two counts answer different questions** (business rule 69). `_count.reservations` is FILTERED to `status: "active"` with an `ipAddress` — addresses actually held, which is what the Networks list's Reservations column shows and what its `utilizationPercent` is a percentage of. `totalReservations` (a `groupBy` scoped by the same WHERE) is every row whatever its status, which is what a cascade delete removes and what the delete / archive confirmations must quote. Released and expired reservations are kept forever (reservationService soft-releases; only `cleanupStaleDnsResolvedReleased` prunes any of them), so the unfiltered count read 300 reservations on a /24 once a DHCP-leased network had churned. A whole-subnet reservation (`ipAddress` null) is excluded from the filtered count deliberately — it holds no individual address and already shows as status `reserved`. The numerator matches ipService.subnetCapacity and the IP panel's footer bar; those three agree or none of them is trusted
...
- **`createSubnet` with no `blockId` places the network in the most specific containing block** (`resolveBlockForCidr`) and 400s when none contains it; a named `blockId` is honoured unchanged. The resolve runs before `createSubnetRowChecked` takes the per-block lock — a block created or deleted between the two is the same window a named `blockId` already had. Returns the row plus `block: { id, name, cidr }` so a caller that named none learns where it landed; both Add Network dialogs (`subnets.js`, `block-panel.js`) rely on that for their toast and preview the choice through `GET /subnets/resolve-block`
...
- **`getSubnetIps`' `toReservationDto` is the ONLY shape the IP panel sees, so a column missing from it is a dead feature in the browser, not a degraded one.** It withheld `vipInfo` and all four push columns until 2026-09: the panel's VIP badge had therefore never rendered for anyone, and its "Queued for push" / "Push failed" rungs and the Retry button they gate could not fire, so a reservation queued against an unreachable gate drew as an ordinary active row. Both were found by reading the DTO, not by anything failing. Adding a Status-column or row-action behaviour to `public/js/ip-panel.js` means checking this function first.
...
**When changing this:**
- **Adding a field to `listSubnets`' rows costs one query for the whole page, or 2000.** Its three queries (rows, total, the `totalReservations` groupBy) are each one round trip whatever the page size, and the default limit is 10000 — a per-row count or a per-row `usableHostCount` inside a loop that hits the DB turns one aggregate into a query per network. Scope an aggregate with a relation filter (`where: { subnet: where }`) rather than an `IN` list of the page's ids, which at the max limit is 10000 UUIDs in one statement
