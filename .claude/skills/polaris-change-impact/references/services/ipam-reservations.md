...
- A group qualifies through ONE of TWO clauses, and `DuplicateIpGroup.qualifiedBy` records which (`"asset-type"` | `"cross-source"`), carried onto `proposedAssetFields.qualifiedBy` so the card can pick its explainer.
...
- `qualifiedBy` is absent on conflicts raised before the cross-source clause shipped; the card treats a missing value as `"asset-type"`. Don't make it required without a backfill.
...
- **A `vip` row is not skipped: it LEARNS the DHCP fact** via `decideVipDhcpBinding`, which writes `dhcpBinding` and a fill-only `macAddress` and never `sourceType` or `expiresAt` (business rules 23 and 76). This is what puts "VIP / Leased" in the panel's Status column.
