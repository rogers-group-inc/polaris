...
**The `http` type's `form` mode (2026-09, business rule 87)** is a DEVICE ADMIN LOGIN — a switch or access point's own web-UI username + password — held for the firmware repository and nothing else. `validateHttpConfig` keeps both carriers for it (like basic / digest) and strips the token; it is NOT an HTTP auth scheme, so `manufacturerProfileService.httpFieldsForWrite` refuses it on an HTTP-check widget (400), `monitoringService.probeHttp` sends no header for it (guessing Basic would post the device's admin password to whatever answered), and `POST /credentials/test` answers as a result that it is verified by the upgrade engine signing in. `utils/httpCheck.ts → isDeviceLoginCredential` is the one predicate every consumer reads. The only reader of the secret is `firmwareRepositoryService.resolveFirmwareCredential({ revealSecrets: true })`.
