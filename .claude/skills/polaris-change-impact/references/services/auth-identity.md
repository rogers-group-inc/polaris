...
**When changing this:**
- Re-run `tests/unit/sshOnboardingScript.test.ts` (77 cases; injection attempts, both account modes, firewall on/off, the profile the rule lands on and what happens to Windows' own rule, predicate sharing, the `New-LocalUser` parameter limits, the account + Administrators checks in both halves, the sshd start-failure diagnostics).
