...
**When changing this:**
- Re-run `tests/unit/sshOnboardingScript.test.ts` (70 cases; injection attempts, both account modes, firewall on/off, predicate sharing, the `New-LocalUser` parameter limits, the account + Administrators checks in both halves).
