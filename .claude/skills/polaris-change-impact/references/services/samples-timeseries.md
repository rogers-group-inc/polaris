...
## services/dbSizeService.ts

**What it owns:** Every "how big is this?" figure about the database, from ONE attribution pass over `pg_class`: the per-public-table sizes behind the Maintenance tab's table list and the capacity snapshot's sample-table stats, and the bucket breakdown (`polaris` / `pgboss` / `catalog` / `other`) behind its "Current size". Catalog-only by rule — `pg_class.relpages × block_size`, never `pg_database_size()` or `pg_total_relation_size()`, which stat() every relfilenode behind a relation and were minutes-slow at prod chunk counts.

**Public API:** `getTableSizes(names?)` → `{ sizing, tables[] }`, `getDatabaseSizeBreakdown()` → `DatabaseSizeBreakdown`, the pure `summarizeBuckets`, and `buildTableSizeSql` + `_resetCatalogShapeCache` for the tests.

**Cross-service deps:** `timescaleService.isTimescaleAvailable()` (whether to attempt the chunk fold).

**Used by:** `src/services/capacityService.ts → getSampleTableStats()` — per-sample-table bytes/rows/dead-tuples for the snapshot and the steady-state projection's `baseBytes` subtraction; `src/services/capacityService.ts → getCapacitySnapshot()` — `database.sizeBytes` + `database.sizeBreakdown`; `src/api/routes/serverSettings.ts → GET /server-settings/database` — the table list, the total, and the `sizeAccounting` block the card reconciles with.

**Invariants:**
- **The buckets SUM to the total.** `totalBytes` is computed as `polaris + pgboss + catalog + other`, not queried separately, so the Database card's figure and the explanation under it cannot drift apart. This module exists because they did: the card printed a whole-database `SUM(relpages)` over EVERY schema above a table list that was public-only, heap+index-only and (see below) hypertable-parent-only — 76.6 GB over rows summing to ~2.6 GB on prod, 2026-09-17.
- **A relation is attributed to exactly one bucket, via the TABLE it serves.** An index follows the table it indexes; a TOAST relation follows the table it stores for; a TOAST index takes two hops. The pg_class self-joins that resolve this are plain `LEFT JOIN`s, never LATERAL subqueries — there is no index on `reltoastrelid`, so a per-row lookup is a seq scan of pg_class per TOAST relation, i.e. quadratic in the chunk count.
- **A table's bytes include its TOAST.** Not an edge case for hypertables: a COMPRESSED chunk keeps its compressed batches in TOAST, which is most of its size (10.6 MB of a 32.4 MB test hypertable). The predecessor queries counted heap + indexes only, so the total included `relkind = 't'` and no row ever did.
- **Chunk relations are found by NAME PREFIX (`_hyper_<htId>_<chunkId>_chunk…` / `compress_hyper_<htId>_…`), not through `_timescaledb_catalog.chunk`.** TimescaleDB 2.30 replaced that table's `schema_name`/`table_name` with a single `relid` (so the old join failed outright with 42703) AND stopped registering compressed chunks anywhere: `compression_state` is 0, `compressed_hypertable_id` is NULL, `compression_chunk_size.compressed_chunk_id` is 0, and pg_depend holds no link — nothing in the catalog points at a compressed chunk relation. The naming convention is the only stable route and needs only `hypertable.id`, which has never moved. Verified against `hypertable_detailed_size()` on 2.30: 31.94 MB measured vs 32.40 MB reported, the 1.4% being the FSM/visibility-map forks `relpages` does not count.
- `hypertable.compressed_hypertable_id` is probed for once per process and joined only when present — pre-2.30 needs that hop (a compressed chunk's name carries the INTERNAL compression hypertable's id), and referencing a column a future version drops would error the whole query rather than degrade one row.
- **The fold failing is RETURNED, not just logged.** `sizing: "parent-only"` reaches the card, because a 15 GB sample table and an empty one both render as "0 B" once the fold is gone — which is exactly why the prod symptom survived a PG17 upgrade unnoticed. `"no-timescale"` is the third, legitimate state.
- Compressed chunks contribute BYTES but not ROWS: a compressed chunk's `n_live_tup` is its batch count (~1 per 1000 logical rows), so counting it would understate the row column while the bytes are counted in full. A heavily-compressed hypertable therefore reports far fewer rows than it holds — a known, pre-existing limitation of the row column, not of the sizes.
- `neverAnalyzedRelations` (`reltuples = -1`) is reported with every breakdown because relpages is only as fresh as the last VACUUM/ANALYZE. A `pg_upgrade` does not carry planner stats before PG18, so an un-analyzed relation reports zero pages regardless of content and every figure here understates until `vacuumdb --analyze-only` runs — and autovacuum never does it for a relation nothing writes to. `measureNeverAnalyzed()` therefore skips empty heaps (`pg_relation_size(main) = 0` — every TimescaleDB compression leaves a truncated, never-analyzed shell, and counting those warned permanently on a healthy install) and reports `neverAnalyzedMissingBytes` (on-disk size less the pages the catalog already credits). The stat() is only paid when at most `NEVER_ANALYZED_MEASURE_CAP` (200) relations are un-analyzed; above it the bytes are `null` and the count is raw, because measuring a restored database relation by relation is the filesystem walk this service exists to avoid. The card weighs the bytes, not the count: a warning over 64 MB or 1% of the total, a plain note below.

**When changing this:**
- Scale: one round trip per figure, hash joins over the catalog, no per-table or per-asset query. At 2000 monitored assets pg_class holds the 28 hypertables' chunks plus their indexes and TOAST — tens of thousands of rows, all in shared buffers. Never add a query inside a loop over tables or chunks: this runs on a tab fetch.
- Verify against a real catalog, not only mocks — `tests/integration/dbSize.test.ts` asserts the buckets equal the flat whole-database `SUM(relpages)`, that the table list plus the unlisted buckets equals the total, that a TOASTed column's bytes land on its table (within 5% of `pg_total_relation_size`), and that a hypertable's chunk bytes land on the hypertable (within 10% of `hypertable_size`). A change that silently stops folding still passes the unit tests.
- A TimescaleDB major upgrade is the risk event for this module. Check the chunk relation naming and re-run the integration suite against the new version before assuming the fold still works.
...
**Cross-service deps:** `monitoringService` (cadences, retention), `timescaleService` (hypertable check), `dbSizeService` (ALL catalog sizing — `getTableSizes()` for the per-sample-table stats, `getDatabaseSizeBreakdown()` for `database.sizeBytes` + `database.sizeBreakdown`), `queueService` (pg-boss installed, boot/persisted mode), `deploymentContext` (DB co-location), `capacityDbIo` (pure disk-I/O rate math), `capacityAdvisorService` (dynamic import in `getCapacitySnapshotWithAdvisor`; type-only the other direction to avoid runtime cycle).
...
**Invariants:**
- Severity tiers: **critical** = disk <10%, DB >50% of free disk, stale autovacuum >7d on sample table (Timescale hypertables exempt — append-only chunks legitimately don't autovacuum), projected >8× RAM; **warning** = disk 10–20%, projected >4× RAM, dead-tup >20%, pgTuningNeeded, max_connections_undersized (advisor-driven), db_io_pressure (avgBackendsBlockedOnDisk ≥ 2.0); **watch** = disk 20–30%, db_io_pressure (avgBackendsBlockedOnDisk ≥ 0.5 — measured disk-read wait from pg_stat_database.blk_read_time, guarded on DB > host RAM; replaced the old size-based ram_insufficient heuristic), track_io_timing_off (DB > RAM but track_io_timing off so wait can't be measured — own family `pg_io_timing`), db_pool_undersized (>=80% of pool capacity), monitor_workers_undersized (advisor-driven rollup), monitor_handler_timeout_pressure (advisor-driven; p90 ≥ 70% of `pgboss.queue.expire_seconds` on any monitor cadence), timescale_recommended (extension not installed — watch always, warning once sample tables pass 1 GB; TimescaleDB is required as of 2026-09-11, business rule 52), metrics_token_unset, health_token_unset; **ok** = none.
...
- **A volume that cannot be measured DEGRADES, it does not disappear.** `statfsPath` delegates to `startupDiskCheck.statfsWithAncestorFallback()`, which walks up to the nearest reachable ancestor when the path itself is unreachable, and sets `VolumeStat.measuredVia` so the degradation is visible rather than implied. PGDATA and its parents are mode 0700 `postgres` on a PGDG install and Polaris runs as the unprivileged app user, so `statfs()` on the data directory is EACCES on a least-privilege host. That error used to be swallowed by a bare `catch` and the DB volume dropped out of the list entirely — on prod (2026-09) `/var` filled to 100% and `pg_upgrade` died of ENOSPC while the card still reported "All capacity checks passed", because the filesystem that filled was never graded. `statfs()` is mount-level, so an ancestor on the same mount is exact; it is wrong only for a PGDATA that is its own mount beneath an unreadable parent, which is why both paths are kept.
- **"Is the DB on this host" is `deploymentContext.dbIsLocal()` and nothing else.** This module carried a private copy matching only the three loopback literals, so a `DATABASE_URL` naming the host's own FQDN or LAN address — or a socket URL with no host — reported the DB as remote, skipping the DB volume AND mislabelling `dbColocated` on the card. `startupDiskCheck` was moved onto the shared helper in the 2026-08 audit and this copy was missed.
- **This module no longer measures anything itself: every byte comes from `dbSizeService`.** `database.sizeBytes` keeps its old meaning — EVERY relation in the database, all schemas — and that is deliberate: the disk-overflow reasons compare it against free space and the projection subtracts the measured sample tables from it, so scoping it to Polaris's own tables would quietly drop pg-boss's bytes out of both while they occupy the same disk. The operator-facing split lives beside it in `database.sizeBreakdown` (`polaris` / `pgboss` / `catalog` / `other` + `neverAnalyzedRelations`), whose buckets sum to `sizeBytes` by construction.
- **The sample-table measurement now sees compressed chunks and TOAST.** It used to walk `pg_inherits`, which cannot reach a compressed chunk at all, so compressed sample bytes were never subtracted from the database size and rode into the projection through `baseBytes` at face value — the same class of bug as a hypertable missing from `SAMPLE_TABLES`, just quieter, and growing with compression coverage. Expect the projection to read slightly LOWER than before on a compressed database: those bytes are now forecast instead of carried.
...
- Sample table rows-per-asset-per-day (the fallback model): conservative defaults (e.g., asset_monitor_samples = 86400/intervalSeconds) when no samples yet. **`asset_perf_sla_samples` detail is measured-only** (2026-09-23: `countKey: null`, no `DEFAULT_ROWS_PER_ASSET_PER_DAY` entry): SD-WAN runs on a per-INTEGRATION `sdwanIntervalSeconds` and only where `pullSdwan` is on, neither of which is a fleet-wide workload input, and pricing every system-info asset at 60 s would add ~5,760 rows/asset/day to installs that never enable SD-WAN. It projects its real byte-rate once rows exist and zero while the stream is off; its hourly/daily rollups keep the fixed-bucket model.
...
- **A transition writes ONE OF TWO actions, and which one is load-bearing.** A landing back at `ok` writes `CAPACITY_RECOVERED_ACTION` (`capacity.severity_recovered`); an escalation — and a partial recovery that stops at a still-degraded severity — writes `CAPACITY_CHANGED_ACTION` (`capacity.severity_changed`). Both constants are exported, both Events are `resourceType: "system"` with NO `resourceName` (so `utils/alertSubject.ts → eventSubjectLabel()` resolves both to "Polaris server", the subject key an event-mode reset matches on). The split is not cosmetic: an automation's event reset matches on actionPattern + resourceType alone — no `detailsMatch` — so one verb for both directions would have the baseline "Capacity severity escalated" rule (trigger ← changed with `detailsMatch direction=escalated`, reset ← recovered) clear the alert with its own escalation. `platformLifecycleService.ts` splits its verbs for the same reason. Adding a resourceName to one side, or making a partial recovery write the recovered verb, breaks the pair silently — the alert either never clears or clears while the disk is still filling.
...
- Confirm Event emission only fires once per severity change (no duplicate "critical" events on each tick), and that the verb still splits at `ok` — `tests/unit/capacityTransition.test.ts` pins both directions, the partial recovery, and the shared subject.
- Test fallback PG data directory candidates on RHEL/Windows when `SHOW data_directory` fails (non-superuser app role).
- Test the volume scan against an UNREACHABLE data directory, not just a missing one — the permission case and the absent case fail differently and only the absent one is easy to produce. `tests/unit/startupDiskCheckAncestor.test.ts` drives it through a mocked `node:fs/promises`; the real-filesystem half is in `tests/unit/startupDiskCheck.test.ts`. A volume silently missing from the card is the signature of this class of bug: the card grades what is in the list, so an omitted filesystem reads as healthy.
...
- **Not every detail column is worth rolling up, and a deliberate omission has to be recorded as one.** `AssetTelemetrySample.cpuCorePcts` (the per-core CPU vector, from the agent or vCenter) is detail-tier ONLY: element-wise averaging a variable-length jsonb array per asset-bucket means unnesting ~60 rows × the core count for every asset every hour — at 2000 assets and 64 cores that is millions of rows per hourly pass, to serve a view that is used for live diagnosis over hours, not trend analysis over a year. The consequence is carried by the UI instead: `_renderCpuChart` charts the aggregate alone on a rollup tier and says why.
- **A band that is read STACKED rolls up as AVG only**, never min/max: a stack of independent minima sums to a total no sample ever had. Both telemetry band sets take avg — the agent's (`memBuffersBytes` / `memCachedBytes` / `memFreeBytes` / `swapUsedBytes`) and vCenter's (`memPrivateBytes` / `memSharedBytes` / `memBalloonedBytes` / `memSwappedBytes` / `memCompressedBytes` / `memConsumedBytes`); `swapTotalBytes` takes last-seen like `memTotalBytes`, since a machine's installed size is effectively constant and tracking its average makes the chart's axis wobble. Note `AVG` ignores NULLs while `sampleCount` counts every row, so a bucket mixing a breakdown-reporting source with one that reports none under-sums the total — acceptable only because an asset has exactly one telemetry source at a time, so a mixed bucket means the source changed mid-hour.
...
- `readSdwanMembers` returns `{ members, collectedAt }` — `collectedAt` is the newest of the latest per-(member, health-check) perf-SLA samples, one stamp for the whole table, null when the gate has never produced one. The SD-WAN tab dates itself off THAT, not an asset anchor: SD-WAN used to be one optional leg of the system-info pass whose collector swallowed its own failures, so the pass stamp advanced on a scrape that wrote nothing; since 2026-09-23 it has its own cadence, and `lastSdwanAt` is stamped even on a failed read (business rule 13), so an anchor still can't date the rows. The paired `pollIntervalSec` comes from `monitoringService.resolveSdwanPollIntervalForAsset` — the SD-WAN interval the scheduler applies, null when nothing polls the gate.
...
- **A jsonb column is the one place the read is a BOUNDARY, not a cast.** `cpuCorePcts` reaches Prisma as `JsonValue` — the database enforces nothing about its shape — so `coerceVector`-style narrowing (`coreVector` here) is where an older agent, a hand-written row or a future shape change is actually checked. Anything that is not a finite-number array becomes null (charted as "no per-core data") rather than reaching the browser as a ragged array that renders as `NaN` coordinates in an SVG path.
- **Absent and null are different on a tiered reader, and the UI reads the difference.** `readTelemetryHistory` returns `cpuCorePcts: null` on the detail tier (the tier CAN carry it; this source does not report it) and OMITS the key entirely on the rollup tiers (the tier cannot carry it at all). The CPU chart words its note differently for each, so collapsing the two would tell a FortiGate operator to pick a shorter range.
...
**When changing this:**
- The browser-side "reported through" test lives in `_outageMarkers` / `_seriesReportedThrough`, not here: a window with a sample inside it, or with no hole around it, is dropped, because the Polaris Agent pushes on its own schedule and is not gated on `monitorStatus`, so an agent host can keep reporting CPU straight through an outage of the server-side probe transport.
...
- `src/services/monitoringService.ts:recordTelemetryResult` — `enqueueTelemetrySample` (CPU/memory only). Passes through whatever the collector produced and writes the rest as explicit `null`. Since 2026-09 the vCenter collector fills `cpuCorePcts` and the vSphere band set here; the agent's OS bands never pass through this function at all (its push path writes the buffer directly), so they stay null on every row this writes.
...
- `src/services/monitoringService.ts:recordSystemInfoResult` — `enqueueInterfaceSamples`, `enqueueStorageSamples`, `enqueueIpsecTunnelSamples` (SD-WAN no longer rides this pass). Also folds EVERY scraped interface MAC into `AssetMacAddress` (source="monitor-interface") via `reconcileInterfaceMacs` — contiguous MACs coalesce into `[mac, macEnd]` range rows; the replace is scoped to that source, and `reconcileMacAddresses` symmetrically filters monitor-interface entries + scopes its deletes, so the discovery and interface-fold writers never churn each other's rows.
- `src/services/monitoringService.ts:runSdwanFor` — `enqueuePerfSlaSamples` via `persistPerfSlaSampleStream` on the SD-WAN cadence (2026-09-23; gated by `Integration.config.pullSdwan`, every `sdwanIntervalSeconds`; perfSla rows are stamped cadence="fast"). SD-WAN **rules** are persisted in the same flow via `persistSdwanRules` (current-state delete-replace, NOT buffered; skipped when the CMDB read failed).
...
- **A NULLABLE `Json` column cannot take a bare `null` through Prisma**, which reserves that spelling for "leave the field alone" — the only way to write a SQL NULL is the `Prisma.DbNull` sentinel. Translate it in `writeBatch`, not in the row interface: `TelemetrySampleRow.cpuCorePcts` stays `number[] | null` so every enqueue site (the agent push path, `recordTelemetryResult`) keeps writing plain `null` and stays out of the ORM's null vocabulary, and the single `.map()` before `createMany` is the only place that knows.
- **Widening a row type is a change to EVERY writer, including the ones with nothing to say.** `recordTelemetryResult` (FortiOS / SNMP / WinRM / vCenter / SSH) has to pass the agent-only telemetry columns as explicit `null` rather than omitting them — the typecheck at the `createMany` call site is what catches the omission, which is the whole reason the row types are declared locally instead of imported from Prisma.
