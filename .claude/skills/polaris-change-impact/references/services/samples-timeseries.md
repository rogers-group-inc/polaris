...
- `src/services/monitoringService.ts:recordTelemetryResult` — `enqueueTelemetrySample` (CPU/memory only). Writes the agent-only columns — `cpuCorePcts` and the memory-breakdown bands — as explicit `null`: none of the transports this function serves can produce them.
