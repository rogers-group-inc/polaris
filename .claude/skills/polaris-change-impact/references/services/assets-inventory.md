...
**Used by:** `src/services/discovery/discoveryEngine.ts` — the `sweepEndpointGhostsInto` helper called from the FortiSwitch + FortiAP loops of `syncDhcpSubnets` (both update and create branches; candidates = base-MAC lookup + hostname==serial lookup, never bare IP); `src/jobs/mergeFortiswitchEndpointGhosts.ts` (one-shot startup sweep for the legacy NULL-MAC shape); `src/jobs/mergeDuplicateHostnameAssets.ts` — both deciders, serial pass then hostname pass; `mergeDuplicateHostnameGhost` is the HOSTNAME pass's executor only. The serial pass executes through `duplicateSerialConflictService.absorbDuplicateSerialGroup` (→ `assetMergeService.mergeAssets`) since 2026-09-22: `mergeDuplicateHostnameGhost` lets the delete cascade the ghost's `AssetSource` rows and `ManagedAgent`, which is right for a placeholder ghost and destroys an authoritative duplicate-serial record's directory identity and agent enrolment.
...
- **`AssetIndex` normalizes EVERY identity key, and `add` / `remove` / `find*` must agree** (`services/discovery/discoveryEngine.ts`, exported for tests). MAC upper-cases, hostname lower-cases, and serial goes through `utils/fortinetParentKey.ts → normalizeSerialKey` (trim + upper) — the same key rule 83's duplicate-serial sweep groups on. Serial was the ONE key stored verbatim until 2026-09, and the cost is not a failed lookup but a DUPLICATE ASSET: the FortiSwitch/FortiAP/firewall loops try serial FIRST, their next rung is MAC (null on a FortiSwitch until the 2026-08 baseMac capture), and a miss on all three rungs creates a second row for a device Polaris already had. Prod carried exactly that — one switch on two Asset rows, the older holding no `AssetSource` because the newer one's claim on the unique `(fortiswitch, serial)` key re-bound it away. The three Fortinet RMA guards (firewall HA member / switch / AP: "matched by name but carries a DIFFERENT serial ⇒ don't bind") use the same helper, because comparing them with a bare `.toUpperCase()` while the index trims would let a padded serial match the index and then fail the guard — landing back on a fresh asset. Adding a new identity key, or a new serial comparison anywhere in discovery, means using these helpers rather than an inline `.toUpperCase()`.
...
**Public API:** pure — `isUsableSerial(raw)` (RE-EXPORTED from `utils/serialNumber.ts`, which now owns the list, the length floor and the repeated-character shape; this service was the first caller and is no longer the only one), `controllerKeyFor(claim)`, `claimFoldKey(claim, ctx)`, `groupContestedSerials(claims, cutoff, ctx?)`, `claimantSetKey(claimants)`, `groupDuplicateSerialAssets(rows)`, `memberSetKey(members)`, `pickPrimaryMemberId(members)`, `resolveMergeTargets(members, survivorId, rawAbsorbIds)`, `freshnessCutoff(now?)`, `conflictSerialOf(conflict)`, `conflictClaimantsOf(conflict)`, `conflictMembersOf(conflict)`, `serialClaimRejectMessage(conflict)`, `duplicateSerialRejectMessage(conflict)`; DB — `recordControllerClaims(inputs)`, `pruneStaleControllerClaims(now?)`, `loadFreshControllerClaims(cutoff)`, `loadControllerContext(claims)`, `loadDuplicateSerialAssets()`, `reconcileSerialConflicts(now?)` (the job's entry point), `mergeDuplicateSerialAssets(conflict, survivorAssetId, absorbIds, actor)`, `repointDuplicateSerialConflicts(fromAssetId, toAssetId)` + `settleDuplicateSerialConflictsAfterMerge({ survivorAssetId, absorbedAssetId, survivorLabel, actor })` (the two hooks `POST /assets/:id/merge` calls around `mergeAssets`), `absorbDuplicateSerialGroup(canonical, ghosts, actor)` (the automatic serial merge's executor), `logSerialClaimDismissal(conflict, actor)`, `logDuplicateSerialDismissal(conflict, actor)`, `logScanFailure(err)`. Constants `SERIAL_CLAIM_COLLISION_REASON`, `DUPLICATE_SERIAL_COLLISION_REASON`, `CLAIM_FRESH_DAYS`, `CLAIM_PRUNE_DAYS`, `CLAIM_EXCLUDED_STATUSES`, `MAX_PLAUSIBLE_DUPLICATES`, `EMPTY_CONTROLLER_CONTEXT`.

**Cross-service deps:** `prisma`, `logEvent` (eventLogService), `mergeAssets` (assetMergeService — the merge verb delegates rather than re-implementing an absorb), `repointDuplicateIpConflicts` + `reconcileDuplicateIpForAddresses` (duplicateIpConflictService — the automatic merge settles a rule 40(i) card the same way the route does), `utils/fortinetParentKey` (`buildInfraParentIndex` / `resolveInfraParentAsset` / `normalizeSerialKey` / `normalizeNameKey` — controller resolution goes through the shared resolver, never a hostname compare).

**Used by:** `src/jobs/detectSerialConflicts.ts` (30-min sweep, scheduler role), `src/services/discovery/discoveryEngine.ts` (`syncDhcpSubnets` — the FortiSwitch and FortiAP loops collect claims through a local `noteControllerClaim` and flush once with `recordControllerClaims` before Phase 4), `src/api/routes/conflicts.ts` (the duplicate-serial branch of `POST /:id/merge`), `src/api/routes/assets.ts` (`POST /:id/merge` — repoint before, settle after), `src/jobs/mergeDuplicateHostnameAssets.ts` (the serial pass → `absorbDuplicateSerialGroup`), `src/services/conflictResolutionService.ts` (imports both reason constants + both dismissal loggers for the accept-refusal and reject branches). Frontend readers: `renderSerialClaimConflictCard` / `renderDuplicateSerialConflictCard` in `public/js/events.js` and the reason labels in `public/js/widgets/conflictQueue.js`.
...
- A serial only counts when `isUsableSerial` accepts it (SMBIOS placeholders, < 4 chars, single repeated character) — the SAME test `utils/assetProjection.ts` applies to every `SERIAL_RULES` pick and the agent applies before it sends, so by the time this sweep runs a placeholder should never have become an `Asset.serialNumber` in the first place; this remains the backstop for rows written before that guard existed and for any source that bypasses the projection. A duplicate group larger than `MAX_PLAUSIBLE_DUPLICATES` (8) is treated as a vendor default rather than a device recorded nine times.
...
- **A merge made anywhere else must settle the card too.** The card's own *Review & merge...* opens the asset Merge modal, which merges through `POST /assets/:id/merge`, not the conflict verb — and until 2026-09-22 that left the card listed, showing the deleted record, until the next 30-minute sweep closed it as `system:auto-resolved` (or cascade-deleted it outright, unaudited, when the card was filed on the absorbed asset). The route now calls `repointDuplicateSerialConflicts` BEFORE `mergeAssets` and AWAITS `settleDuplicateSerialConflictsAfterMerge` after it, which closes a card whose serial no longer groups as `accepted` by the operator (the merge WAS the resolution) and refreshes the member snapshot of a 3+ group that still stands. Both verbs share the private `settleDuplicateSerialConflict`, so they cannot disagree about when a card is resolved. A new merge entry point needs the same two calls.
- **The automatic serial merge goes through `mergeAssets`, never `mergeDuplicateHostnameGhost`** (`absorbDuplicateSerialGroup`, 2026-09-22). The serial pass shipped using the hostname pass's placeholder-ghost executor, whose delete cascades the ghost's `AssetSource` rows and its `ManagedAgent`; a duplicate-serial group is usually two AUTHORITATIVE records (Entra/Intune beside a Polaris Agent), so that stripped a directory identity and unenrolled the agent. It also repoints and settles both card flavours (actor `system:duplicate-serial-merge`). When both records carry a `ManagedAgent`, the absorbed one's still cascades — `mergeAssets` moves an enrolment only onto a survivor that has none.
