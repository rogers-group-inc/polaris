...
- Changing `resolveOwningGateContexts` changes THREE surfaces at once (this sweep's ARP scoping, the Last Seen Firewall row, endpoint dependency parents — rule 54). The last of those decides which devices suppress, so a widened match there silences alerts.
