...
async function resolveAssetMetricReadings(trigger: Extract<Trigger, { type: "asset_metric" }>, assets: ScopeAssetRow[], saturated?: Set<string>): Promise<Reading[]> {
  ...
  switch (trigger.metric) {
    ...
    case "connLatencyMs": case "connHttpStatus": case "connOk": case "connTlsDaysLeft": {
      const rows = await prisma.assetConnectivitySample.findMany({
      ...
      const names = await connectivityCheckNames(rows.map((r) => r.checkId));
      const now = Date.now();
      const valueFn = (r: typeof rows[number]): number | null => {
        switch (trigger.metric) {
          // A failed run carries no latency (nothing connected, or it timed
          // out): no reading, not a zero — the failure is connOk's to report.
          case "connLatencyMs": return r.latencyMs ?? null;
          case "connHttpStatus": return r.httpStatus ?? null;
          case "connOk": return r.ok ? 1 : 0;
    ...
    case "connHopCount": {
      ...
      const rows = await prisma.assetConnectivityTraceroute.findMany({
        where: { assetId: { in: ids }, timestamp: { gte: since }, ...(df.checkId ? { checkId: df.checkId } : {}) },
    ...
    case "connFailurePct": {
      ...
      const grouped = await prisma.assetConnectivitySample.groupBy({
      ...
      const names = await connectivityCheckNames([...cells.values()].map((c) => c.checkId));
      const out: Reading[] = [];
...
/** id → name for the connectivity checks a resolver's rows mention. One small
 *  query per resolver call (the table is capped at a few dozen rows). */
async function connectivityCheckNames(ids: readonly string[]): Promise<Map<string, string>> {
  const distinct = [...new Set(ids)];
  if (distinct.length === 0) return new Map();
  const rows = await prisma.connectivityCheck.findMany({ where: { id: { in: distinct } }, select: { id: true, name: true } });
