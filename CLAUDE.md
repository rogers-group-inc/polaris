...
**Polaris** is an **asset management and network monitoring tool**, focused on pulling the systems an operator already runs into one dashboard. Seven integrations — FortiManager / FortiGate, Entra ID / Intune, Active Directory, Windows DHCP, vCenter, Azure Arc — are read into one multi-source asset record, monitored over eight telemetry streams, and acted on by automations. Named after the North Star — a fixed reference point operators can navigate by when wiring up everything else.
...
| Database | PostgreSQL 17 + **TimescaleDB** (required — every install path provisions it; the 28 sample/rollup tables are hypertables) |
...
- **Polaris is manufacturer-agnostic: vendor facts are operator DATA, never code.** Polaris ships the engine and the IETF/IEEE standard MIBs (`src/services/stdMibs/`); a vendor's OIDs come from the MIB the operator uploads and resolve BY SYMBOL NAME through `oidRegistry.ts → resolveOidSync(symbol, { manufacturer, model })`, and which symbol means what is a `ManufacturerProfile` row they can edit. The rule is mechanical and tested: **no enterprise-arc OID literal (`1.3.6.1.4.1.*`) anywhere in `src/`** (`tests/unit/noEnterpriseOids.test.ts`, a burn-down allowlist naming the files that still carry them). Standard arcs may stay numeric, and code may know how a table is SHAPED — never where a vendor put it. An unresolved symbol degrades to "not collected for this asset": never a throw, never a wipe. A profile only earns its place by overriding what the generic MIBs cannot say (business rule 68). Full contract: `polaris-change-impact` → cross-cutting/vendor-snmp-knowledge-boundary.md.
