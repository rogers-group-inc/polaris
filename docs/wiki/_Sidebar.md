### [Polaris](Home)

**Start here**
- [What Polaris is](Home)
- [Concepts and vocabulary](Concepts)
- [Installation](Installation)
- [First-run setup](First-Run-Setup)
- [Getting around](Navigation-and-Account)

**Screens**
- [Dashboard](Dashboard)
- [IPAM](IPAM)
- [Assets](Assets)
- [Device Map](Device-Map)
- [Application Map](Application-Map)
- [Path Monitor](Path-Monitor)
- [Events](Events)
- [Automations](Automations)
- [Integrations](Integrations)
- [Users](Users-Roles-and-Permissions)
- [Server Settings](Server-Settings)
- [Mobile app & Dash wallboard](Mobile-and-Dash)

**Automations**
- [Overview](Automations)
- [Triggers and conditions](Automation-Triggers)
- [Actions and recipients](Automation-Actions)
- [Escalation, reminders, quiet time](Automation-Escalation)
- [Scripts](Automation-Scripts)
- [Path checks](Path-Monitor)
- [Delivery channels](Delivery-Channels)
- [Address book](Address-Book)

**Monitoring**
- [How polling works](Monitoring)
- [Polling methods per stream](Polling-Methods)
- [Monitor states and down detection](Monitor-States)
- [Dependency suppression](Dependency-Suppression)
- [Maintenance windows](Maintenance-Windows)

**Discovery**
- [How discovery works](Discovery)
- [FortiManager & FortiGate](Integration-Fortinet)
- [Entra ID / Intune & Active Directory](Integration-Directory)
- [VMware vCenter](Integration-vCenter)
- [Azure Arc](Integration-Azure-Arc)
- [Windows Server DHCP](Integration-Windows-Server)
- [Polaris Agent](Polaris-Agent)
- [Network Discovery (active scan)](Network-Discovery)

**Reference**
- [Conflict resolution](Conflict-Resolution)
- [Business rules](Business-Rules)
- [REST API](API)
- [Backup and restore](Backup-and-Restore)
- [Updates](Updates)
- [High availability](High-Availability)
- [Troubleshooting](Troubleshooting)
