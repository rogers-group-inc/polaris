...
**Fresh installs only** — on **RHEL / Rocky / AlmaLinux 9**, **Ubuntu / Debian**, or as a container. The only differences between the two Linux paths are package names and where PostgreSQL puts its data directory.

Already running Polaris? Updates are the in-app updater under **Server Settings → Maintenance → Updates**, and the one-time moves it cannot make for you — a new Node or Java major, PostgreSQL 15 → 17, AppStream → PGDG, the legacy single-process layout, putting nginx in front — are [docs/UPGRADING.md](UPGRADING.md).
...
and is below that floor; installs still on it should follow
[docs/UPGRADING.md](UPGRADING.md#upgrading-node-on-an-existing-install). Node 22 is also supported (to ~April 2027) if you are
already on it.
...
end-of-life as of April 2026 — see [docs/UPGRADING.md](UPGRADING.md#upgrading-node-on-an-existing-install).
...
longer shipped (a host still running one moves across via
[docs/UPGRADING.md](UPGRADING.md#upgrading-a-legacy-single-process-install)).
This section documents what the install scripts actually do, for operators
who want to understand the layout or customize it post-install.
...
nginx terminates TLS for every Polaris install, and the setup scripts land this
layout for you (an install that predates it migrates via
[docs/UPGRADING.md](UPGRADING.md#migrating-an-existing-install-to-the-nginx-front-end)).
Polaris itself listens HTTP-only on
...
### The in-app nginx GUI

Server Settings → Web Server shows the in-app nginx GUI: the
...
2. **Set the account, per platform.** Pick the Windows or Linux tab, then either name an existing account or have the script create a dedicated one (Windows: random password; Linux: password locked — either way authentication is by key only). An existing account has **no default**: enter one that is really a local administrator on the endpoints — `DOMAIN\username` or a local name on Windows, the bare name on Linux — and save before downloading or publishing the script, which is refused until you do. A created account left blank is named `polaris-agent`; a Windows one you name yourself is capped at **20 characters**, the limit `New-LocalUser` enforces, and is refused on save rather than failing later on every endpoint. Optionally give the Polaris server's address to scope inbound TCP/22 to it — on Windows, setting it also **disables** the any-source OpenSSH rule Windows creates for itself, so the scoping is real; leaving it blank opens nothing but still extends that rule to the **Domain** profile, without which a domain-joined endpoint runs sshd that nothing can reach (business rule 76). Polaris keeps one managed credential per platform, both holding the same key: a credential carries a single username, and a Windows `DOMAIN\user` is meaningless on Linux.
...
- **Windows** also writes its own firewall rule when the OpenSSH capability installs — `OpenSSH-Server-In-TCP`, any source, **Private profile only**. A domain-joined endpoint is on the Domain profile, so sshd ends up installed, running and unreachable, with a healthy-looking service and an empty event log. (It also means a rule you scope to Polaris narrows nothing while that one is enabled, since firewall rules are additive allows.)
...
**Per-asset:** Asset details modal → System tab → Polaris Agent panel → **Upgrade…** button. Confirm and watch the install-state pill flip `active → upgrading → active` within ~10 s. The agent's bearer + cert pin survive the swap — no re-enrollment, and normally no need to repick a credential: the install credential stored on the row is reused.

**If that row has no credential**, Polaris falls back to the Polaris-managed SSH deployment credential (Integrations → Polaris Agent → SSH Deployment) rather than refusing. A row loses its stored credential when someone deletes that credential, and agents installed before Polaris recorded one never had it — either way the host used to be stuck on its old binary with no way forward but force-remove and reinstall. Because the managed credential is key-only, the fallback also connects over SSH even on a Windows host whose row says WinRM, so the target needs OpenSSH Server running with the deployment key authorized (which is what the onboarding script does). Nothing is written back until the upgrade actually succeeds — a host that only speaks WinRM simply fails to connect and keeps its recorded settings. You can still name a credential explicitly by passing `credentialId` to `POST /api/v1/assets/:id/agent/upgrade`; one supplied that way is used for that run only and never stored.
...
Failures land per-row as `installStatus="upgrade_failed"` with the error captured in `installError`; an audit Event (`agent.upgrade_failed`) goes out per failed host. A failed row is still upgradeable — the old binary is what's still running — so it keeps its out-of-date badge and stays in the eligibility filter: retry an individual host with the Retry Upgrade button on its asset details panel, click Upgrade-all again, or simply leave it, since the next build's auto-upgrade fan-out (when enabled) picks it up along with everything else. A host Polaris cannot even start an upgrade on — no credential on the row and no deployment keypair generated — writes an `agent.upgrade_skipped` Event against the asset naming the version it is stuck on, so it shows up in the Events list instead of being passed over in silence. Already-current hosts are skipped without an Event. That matters most for laptops: a machine asleep or off-VPN during a fan-out fails, then catches up on its own once it's reachable at the next attempt.
