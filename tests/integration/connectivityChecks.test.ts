/**
 * tests/integration/connectivityChecks.test.ts — agent-run connectivity checks
 ...
 *     connectivity.path_changed Event,
 *   - the engine's conn* resolvers read the stored samples (previewRule),
...
const d = dbDescribe;
const HOST = "conn-check-it-host";
const GATE = "conn-check-it-gate";
...
async function cleanup(): Promise<void> {
  await prisma.connectivityCheck.deleteMany({ where: { name: { startsWith: "IT connectivity" } } });
...
d("connectivity checks", () => {
  it("creates a check and reconciles it onto the tagged agent host", async () => {
    ...
    const res = await agent.post("/api/v1/connectivity-checks").set("X-CSRF-Token", csrf).send({
      name: "IT connectivity intranet",
      kind: "https",
    ...
    const src = await prisma.connectivityCheckSource.findMany({ where: { checkId } });
    expect(src.map((s) => s.assetId)).toEqual([hostId]);
    expect(await waitForEventCount("connectivity_check.created", 1, checkId)).toBe(1);
  });

  it("refuses a loopback target", async () => {
    const { agent, csrf } = await authedAgent(app);
    const res = await agent.post("/api/v1/connectivity-checks").set("X-CSRF-Token", csrf).send({
      name: "IT connectivity loopback", kind: "http", target: "http://127.0.0.1/", scope: { allAssets: true },
    });
  ...
  it("ships the check in /agents/config and moves the heartbeat etag on an edit", async () => {
    ...
    expect(cfg.body.connectivityChecks).toHaveLength(1);
    expect(cfg.body.connectivityChecks[0]).toMatchObject({ id: checkId, kind: "https", expectStatus: "200", expectBody: { mode: "contains", value: "ok" } });
    const hb1 = await agentReq("post", "/heartbeat").send({ agentVersion: "0.21.0" });
    const { agent, csrf } = await authedAgent(app);
    const cur = await agent.get(`/api/v1/connectivity-checks/${checkId}`);
    const upd = await agent.put(`/api/v1/connectivity-checks/${checkId}`).set("X-CSRF-Token", csrf).send({
  ...
  it("ships nothing to an agent below 0.21.0", async () => {
    ...
    const cfg = await agentReq("get", "/config");
    expect(cfg.body.connectivityChecks).toEqual([]);
  ...
  it("ingests samples: rejects a foreign check, enforces the excerpt policy, never touches monitorStatus", async () => {
    ...
    const rows = await prisma.assetConnectivitySample.findMany({ where: { assetId: hostId, checkId }, orderBy: { timestamp: "asc" } });
    expect(rows).toHaveLength(2);
    expect(rows[0].bodyExcerpt).toBeNull();
    expect(rows[1].bodyExcerpt).toBe("Service Unavailable");
    const src = await prisma.connectivityCheckSource.findFirst({ where: { checkId, assetId: hostId } });
  ...
  it("the engine's conn* resolvers read the samples", async () => {
    const res = await previewRule(previewInputSchema.parse({
      name: "preview",
      trigger: { type: "asset_metric", metric: "connLatencyMs", aggregation: "max", windowSec: 900, operator: ">", threshold: 500, forDurationSec: 0, dimensionFilter: { checkId } },
    ...
    const fail = await previewRule(previewInputSchema.parse({
      name: "preview2",
      trigger: { type: "asset_metric", metric: "connFailurePct", aggregation: "latest", windowSec: 900, operator: ">", threshold: 10, forDurationSec: 0 },
      scope: { assetIds: [hostId] },
    }) as never);
  ...
  it("resolves traceroute hops to the monitored gate and writes connectivity.path_changed on a changed path", async () => {
    ...
    const r1 = await agentReq("post", "/samples").send({ stream: "connectivityTraceroute", samples: [{ checkId, timestamp: new Date(base).toISOString(), complete: true, destinationIp: "203.0.113.10", hops: hops("203.0.113.10") }] });
    expect(r1.body).toEqual({ accepted: 1, rejected: 0 });
    const tr = await prisma.assetConnectivityTraceroute.findFirst({ where: { assetId: hostId, checkId } });
    const first = (tr!.hops as Array<{ assetId?: string; hostname?: string }>)[0];
    expect(first).toMatchObject({ assetId: gateId, hostname: GATE });
    await agentReq("post", "/samples").send({ stream: "connectivityTraceroute", samples: [{ checkId, timestamp: new Date(base + 1_200_000).toISOString(), complete: true, destinationIp: "203.0.113.10", hops: hops("198.51.100.7") }] });
    expect(await waitForEventCount("connectivity.path_changed", 1, hostId)).toBe(1);
  });

  it("lists the host's checks and history for the slide-over", async () => {
    const { agent } = await authedAgent(app);
    const checks = await agent.get(`/api/v1/assets/${hostId}/connectivity-checks`);
    ...
    const hist = await agent.get(`/api/v1/assets/${hostId}/connectivity-history?checkId=${checkId}&range=1h`);
    expect(hist.status).toBe(200);
    expect(hist.body.samples.length).toBe(2);
    const trs = await agent.get(`/api/v1/assets/${hostId}/connectivity-traceroutes?checkId=${checkId}`);
    expect(trs.body.traceroutes).toHaveLength(2);
    const results = await agent.get(`/api/v1/connectivity-checks/${checkId}/results`);
  ...
  it("rolls the samples up hourly and daily (ok/fail counts, mode status)", async () => {
    ...
    await rollupDaily(2);
    const hourly = await prisma.assetConnectivitySampleHourly.findMany({ where: { assetId: hostId, checkId } });
    const total = hourly.reduce((n, h) => n + h.sampleCount, 0);
    expect(total).toBe(2);
    expect(hourly.reduce((n, h) => n + h.failCount, 0)).toBe(1);
    const daily = await prisma.assetConnectivitySampleDaily.findMany({ where: { assetId: hostId, checkId } });
    expect(daily.reduce((n, h) => n + h.okCount, 0)).toBe(1);
  });

  it("drops the source when the agent is uninstalled (reconcile)", async () => {
    const { reconcileConnectivityCheckSources } = await import("../../src/services/connectivityCheckService.js");
    await prisma.managedAgent.update({ where: { assetId: hostId }, data: { installStatus: "failed" } });
    await reconcileConnectivityCheckSources(checkId);
    expect(await prisma.connectivityCheckSource.count({ where: { checkId } })).toBe(0);
