...
podman exec -d polaris-<slug>_app_1 node /app/scripts/mock-firmware-devices.mjs
```
...
and comes back reporting the image's version. Its stdout (`podman logs` will not show a
`podman exec -d` process; run it in the foreground in a second terminal to watch) prints
every step. Both scripts refuse `NODE_ENV=production`.
